...
const CLIENT_ID =
  "mcp.standard.google_workspace.oauth.client" as LocalCoreCredentialId;
const TOKENS_ID =
  "mcp.standard.google_workspace.oauth.tokens" as LocalCoreCredentialId;
...
export class LocalCoreGoogleWorkspaceOAuthSessionManager {
  ...
  async start(input: {
    ...
    if (
      ...
      throw new Error("Google Workspace already has a connection window open.");
    const clientId = parseClientId(input.clientId);
    const packs = parsePacks(input.packs);
    const priorScopes = await this.#priorGrantedScopes();
    const scopes =
      priorScopes.length === 0
        ? scopesForGoogleWorkspacePacks(packs)
        : resourceScopesForGoogleWorkspacePacks(packs).filter(
            (scope) => !priorScopes.includes(scope),
          );
    if (scopes.length === 0) {
      return {
    ...
    const callbackServer = createServer((request, response) => {
      if (!active)
        return writeCallback(
          response,
          503,
          "This App connection is not ready.",
        );
      void this.#callback(
        active,
        request.method,
        ...
        .then((result) =>
          writeCallback(
            response,
            result.status,
            result.message ?? "Kestrel could not complete this App connection.",
          ),
        )
        .catch(() =>
          writeCallback(
  ...
  async #callback(
    ...
    if (this.#now() >= session.expiresAtMs) {
      ...
      return {
        status: 410,
        message:
          "This Google Workspace connection expired. Return to Kestrel and try again.",
      };
    }
    ...
    try {
      const response = await this.#fetch(
        "https://oauth2.googleapis.com/token",
        {
          ...
          body: new URLSearchParams({
            ...
            code,
            redirect_uri: `http://${session.callbackHost}${session.callbackPath}`,
      ...
      >;
      if (!response.ok)
        throw new Error("Google rejected the authorization exchange.");
      ...
      if (session.scopes.some((scope) => !granted.has(scope)))
        throw new Error("Google did not grant every selected capability.");
      ...
      if (!verification.ok)
        throw new Error(
          "Kestrel could not verify the Google Workspace account.",
        );
      ...
      return {
        status: 200,
        message: "Google Workspace connected. You can return to Kestrel.",
      };
    } catch {
      session.view = {
        ...session.view,
        state: "failed",
        error: "Kestrel could not authorize this Google Workspace connection.",
      };
      session.callbackServer.close();
  ...
  async #priorGrantedScopes(): Promise<string[]> {
    try {
      return (await readGoogleTokens(this.#store)).scope
        .split(/\s+/u)
...
function parseClientId(value: unknown): string {
  if (
    ...
    throw new Error("The Google Workspace client identity is invalid.");
  return value;
}
function parsePacks(value: unknown): GoogleWorkspacePack[] {
  if (
    !Array.isArray(value) ||
    value.length === 0 ||
    value.some(
  ...
  return [...new Set(value)] as GoogleWorkspacePack[];
}
