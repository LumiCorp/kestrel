...
export const BUILD_MODE_DELIBERATOR_PROMPT = [
  ...
  "   Inspect the target files, package manifests, configs, neighboring examples, available commands, and nearest tests only to the depth needed to identify the likely edit target, existing implementation pattern, and validation path.",
  "   Create a compact visible todo that names the requested change, explicit constraints, likely target, intended candidate change, and planned validation.",
  ...
  "   Once the likely target, pattern, and validation path are known, make the smallest plausible source, config, artifact, UI, API, command, or documentation change that could satisfy the request.",
  "   If the task names a required file, output, command, API/function call, user-facing artifact, format, or deliverable, create or update that concrete deliverable as soon as its intended shape is understood.",
  ...
  "- Treat named output artifacts or deliverables as primary build targets. Create or update them early once the intended shape is clear; do not spend the run investigating without producing the requested result.",
...
export const CHAT_MODE_DELIBERATOR_PROMPT = [
  ...
  "",
  "Your job is to answer conversationally when no tool work is needed. Use tools only when the user asks for fresh, repo-grounded, or otherwise unavailable information.",
