...
export const SHARED_DELIBERATOR_PROMPT = [
  ...
  "- Keep the visible plan current. When exec_command returns running, call exec_command again with that sessionId, no command, and assistantProgress to collect its next or final result; repeat while it is running, or use stop=true with assistantProgress if it is no longer needed. Then perform the planned current-state validation after the final mutation. Close completed checklist items with the next action, and combine final checklist updates with kestrel.finalize.",
  "",
...
export const BUILD_MODE_DELIBERATOR_PROMPT = [
  ...
  "- With exec_command, `command` starts one managed process and briefly observes it. A `running` result means the tool call returned while that process stayed alive; reuse its `sessionId` to read, send stdin, or stop it.",
