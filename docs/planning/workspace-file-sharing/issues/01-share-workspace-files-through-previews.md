...
- Extend the hosted message projection and public OpenAPI contracts with the file-share kind, URL, media type, preview ID, size, file count, and expiry. Keep the added fields backward compatible for existing clients. The separate mobile application is not part of this issue.
...
The result-normalization boundary already supports artifact presentations through `SharedToolModule.normalizeResult` and `AgentToolPresentation`. Web rendering is owned by `apps/web/components/chatbot/message.tsx`. Hosted artifact projection is implemented in `apps/web/lib/mobile/message-parts.ts`, with public schemas in `apps/web/openapi/mobile-v1.json` and `apps/web/openapi/mobile-v2.json`.
...
Extend `tests/unit/workspace-preview-tools.test.ts` and `tests/integration/workspace-preview-process-lifecycle.test.ts` or add focused neighboring coverage. Prove real managed-process retention and cleanup, not only mocked publication. Add focused Kestrel One web and hosted-projection tests for the Download presentation and backward-compatible API shape.
...
- The Download card appears in Kestrel One web, and the hosted artifact API exposes the same additive file-share metadata.
