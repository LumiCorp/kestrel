...
- Use the ordinary App relay for serialized control requests and screenshot results up to 20 MiB with its current 30-second timeout. Reject larger payloads without compression, retry, or a fallback transport. Live frames, attachment uploads, and quarantined downloads remain outside the relay. Raw worker addresses and credentials never reach clients.
...
Live viewing belongs to issue 06. File transfers belong to issues 07 and 08. Those issues may implement against the reviewed host contracts before release proof completes, but their final completion still requires the applicable packaged or live host evidence.
