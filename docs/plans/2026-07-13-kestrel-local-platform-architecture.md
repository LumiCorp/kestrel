...
Local Core now also defines a versioned, secret-free runtime-configuration authority. One immutable generation supplies the shared model policy, exact provider endpoint options, the registered Desktop profile, and the environment overlay for each execution bundle. The first bootstrap snapshot is persisted once, and its explicit `environmentOptionsMode` chooses either compatibility inheritance or canonical replacement without inferring authority from the generation number. Legacy model-policy writes are adapters into that store, preserve the selected environment mode, and retire the affected bundle before another run is admitted, preventing profiles and environment values from diverging. The authenticated control API exposes the strict runtime configuration and redacted credential status while Core is blocked, and a narrow repair operation can replace only a malformed persisted configuration; it never exposes credential values. Until the later activation boundary supplies a credential store, inherited provider credentials retain their existing behavior; Keychain selection, credential writes, plaintext migration, and secret-field rejection remain coupled to Desktop's child-runner cutover.
