...
The composer emits a `data-kestrel-file` part for each attachment. The Thread route extracts each file ID and creates the durable turn. Turn creation locks and validates the file rows, checks live Thread grants, enforces count and size limits, persists the user message, and records ordered links in `thread_message_files` ([Thread route](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/app/api/threads/[id]/route.ts), [turn store](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/lib/turns/store.ts)).

The queue sends only the durable turn ID to pg-boss. The Fly worker later reloads the stored messages and acts as the original author ([turn queue](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/lib/turns/queue.ts), [process runtime](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/lib/turns/process-runtime.ts)). This is the correct durable model: stable IDs survive queue delay, retry, and process loss.

The failure begins when the worker assembles runtime input. It extracts attachment IDs from the latest user message and calls `resolveThreadAttachmentsForExecution` ([runtime core](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/lib/agent/kestrel-runtime-core.ts:266)). The file service then creates a storage adapter inside the worker. An R2 adapter returns a signed URL. A local adapter reads the complete object and base64-encodes it ([file service](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/lib/files/service.ts:473)).

The turn-worker contract forbids all `STORAGE_*` configuration ([process contract](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/lib/runtime/process-contracts.ts:78)). That is an intentional least-privilege boundary. However, missing `STORAGE_PROVIDER` silently selects local storage under the current working directory ([storage configuration](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/lib/storage/index.ts:14)). The hosted worker therefore looked for the R2 object under `/workspace/apps/web/.local/storage` and failed with `ENOENT`.

The runtime materializer already accepts a signed URL. It rejects expired or untrusted sources, checks the exact size and SHA-256 digest, stages the file in a private read-only directory, and removes transient source data from the resulting attachment ([runtime materializer](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/src/runtime/attachments/materialize.ts)). This behavior should remain unchanged.

The existing `kestrel.files.open` route proves that the web process can authorize a Thread file and mint a signed URL ([file-open route](/Users/gregasher/.codex/worktrees/2037/kestrel-harness/apps/web/app/api/kestrel/tools/files/open/route.ts:18)). Its authorization contract is for interactive users and environment runtimes. It should not be weakened to accept a generic worker token plus caller-supplied Thread identity.
