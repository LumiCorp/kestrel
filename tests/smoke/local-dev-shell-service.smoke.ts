...
test("LocalDevShellService starts a real supervisor with a sqlite store", {
...
test("LocalDevShellService recovers a real supervisor from a corrupt sqlite store", {
  skip: realSupervisorSmokeSkipReason,
}, async () => {
  const serviceModuleUrl = new URL("../../src/devshell/LocalDevShellService.ts", import.meta.url).href;
  const script = `
...
test("LocalDevShellService starts a real supervisor with explicit split paths", {
  skip: realSupervisorSmokeSkipReason,
}, async () => {
