...
describe("Local Core API process contracts", { concurrency: 2 }, () => {
  ...
  test("Local Core API serves health/status with bearer token auth", async () => {
    ...
    try {
      ...
      assert.equal(
        (await readFile(paths.apiTokenPath, "utf8")).trim(),
        server.token,
      );
      ...
      assert.equal(
        (restoredThread.metadata?.workspace as { workspaceRoot?: string })
          .workspaceRoot,
        home,
      );
      ...
      assert.equal(
        Number.isFinite(Date.parse(runnerErrorEvent.ts ?? "")),
        true,
      );
      assert.equal(
        runnerErrorEvent.commandId,
        "local-core-desktop-invalid-actor",
      );
      ...
      const bundleResponse = (await client.supportBundle()) as {
        supportBundle?:
          | {
                ...
                | {
                    ...
                    socketPresence?:
                      | {
                          ...
                          apiSocketPresent?: boolean | undefined;
                        }
                      | undefined;
                  }
                | undefined;
              extra?:
                | {
                    legacyState?: { coreHome?: string | undefined } | undefined;
                  }
                | undefined;
            }
          | undefined;
      ...
      assert.match(
        bundleResponse.supportBundle?.runtime?.home?.homePath ?? "",
        /kestrel-core-api-/u,
      );
      assert.equal(
        bundleResponse.supportBundle?.runtime?.manifest?.coreVersion,
        "0.6.0",
      );
      assert.equal(bundleResponse.supportBundle?.runtime?.dbMode, "pglite");
      assert.equal(
        "migrations" in (bundleResponse.supportBundle?.runtime ?? {}),
        true,
      );
      assert.match(
        bundleResponse.supportBundle?.runtime?.socketPresence?.apiSocketPath ??
          "",
        /core\/api\.sock$/u,
      );
      assert.equal(
        bundleResponse.supportBundle?.runtime?.socketPresence?.apiSocketPresent,
        true,
      );
      assert.match(
        bundleResponse.supportBundle?.extra?.legacyState?.coreHome ?? "",
        /kestrel-core-api-/u,
      );
      ...
      await assert.rejects(
        () => unauthorized.status(),
        (error) =>
          error instanceof LocalCoreApiError && error.statusCode === 401,
      );
      await assert.rejects(
  ...
  test("Local Core API replaces an idle Desktop restart authority", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-desktop-restart-"),
    );
    const server = await startLocalCoreApiServer({
      env: { KESTREL_CORE_HOME: home },
      platform: "darwin",
    ...
    const paths = resolveLocalCorePaths(home);
    let replacement:
      | Awaited<ReturnType<typeof startLocalCoreApiServer>>
      | undefined;
    try {
      ...
      if (originalLock.state !== "live")
        assert.fail("original Core owner is required");
      const client = new LocalCoreClient({
        socketPath: server.socketPath,
        token: server.token,
      });
      ...
      if (replacementLock.state !== "live")
        assert.fail("replacement Core owner is required");
      assert.notEqual(
  ...
  test("Local Core API makes existing state and Core authority roots private before serving", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-private-"),
    );
    const paths = resolveLocalCorePaths(home);
  ...
  test("Local Core API does not adopt another authority instance in the same process", async () => {
    ...
    try {
      ...
      await writeFile(
        paths.apiSocketPath,
        "preexisting-authority-sentinel\n",
        "utf8",
      );
      ...
      assert.equal(
        await readFile(paths.apiSocketPath, "utf8"),
        "preexisting-authority-sentinel\n",
      );
      assert.equal(
  ...
  test("Local Core keeps control authority reachable and resets a broken startup store", async () => {
    ...
    try {
      ...
      const canonicalPaths = resolveLocalCorePaths(
        await realpath(paths.stateRootPath),
      );
      await store.executor.query("DROP TABLE runner_protocol_events");
      await writeFile(
        path.join(paths.pgliteDataPath, "reset-sentinel"),
        "archived\n",
        "utf8",
      );
      await writeFile(
        path.join(home, "runtime.db"),
        "legacy-runtime\n",
        "utf8",
      );
      await mkdir(paths.settingsPath, { recursive: true });
      await writeFile(
        path.join(paths.settingsPath, "preserve-sentinel"),
        "settings\n",
        "utf8",
      );
      await mkdir(paths.workspaceRegistryPath, { recursive: true });
      await writeFile(
        path.join(paths.workspaceRegistryPath, "preserve-sentinel"),
        "workspaces\n",
        "utf8",
      );
      ...
      const authorityBefore = JSON.parse(
      ...
      assert.equal(
        await readFile(
          path.join(recovered.reset.archivedStorePath, "reset-sentinel"),
          "utf8",
        ),
        "archived\n",
      );
      await assert.rejects(
        readFile(path.join(paths.pgliteDataPath, "reset-sentinel"), "utf8"),
        { code: "ENOENT" },
      );
      assert.equal((await stat(paths.pgliteDataPath)).isDirectory(), true);
      assert.equal(
        await readFile(path.join(home, "runtime.db"), "utf8"),
        "legacy-runtime\n",
      );
      assert.equal(
        await readFile(
      ...
      assert.deepEqual(
        ((await client.runs()) as { runs?: unknown[] }).runs,
        [],
      );
      assert.equal(
        JSON.stringify(await client.supportBundle()).includes(
          recovered.reset.archivedStorePath,
        ),
        false,
      );
      ...
      try {
        assert.deepEqual(
          await sdk.ping(
            ...
            {
              actor: {
                actorId: "runtime-store-reset-test",
                actorType: "operator",
              },
            },
          ),
  ...
  test("Local Core repairs malformed runtime configuration while execution is blocked", async () => {
    ...
    try {
      ...
      assert.equal(
        existsSync(path.join(paths.stateRootPath, "profiles.json")),
        false,
      );
      await assert.rejects(
        () =>
  ...
  test("Local Core runtime-store reset requires explicit confirmation and exposes typed errors", async () => {
    ...
    try {
      for (const body of [
        {},
        { confirm: true, path: "/tmp/not-authoritative" },
      ]) {
        await assert.rejects(
  ...
  test("Local Core runtime-store reset refuses external database authority without mutation", async () => {
    ...
    try {
      ...
      await writeFile(
        path.join(paths.pgliteDataPath, "dormant-sentinel"),
        "untouched\n",
        "utf8",
      );
  ...
  test("Local Core runtime-store reset rejects an active execution before archiving", async () => {
    ...
    try {
      await writeFile(
        path.join(paths.pgliteDataPath, "active-run-sentinel"),
        "present\n",
        "utf8",
      );
      ...
      const blockedUpdate = await withTimeout(
        client.shutdownForDesktopUpdate(),
      );
      ...
      const blockedRestart = await withTimeout(
        client.shutdownForDesktopRestart(),
      );
      ...
      const blockedCodeUpdate = await withTimeout(
        client.shutdownForCodeUpdate(),
      );
  ...
  test("Local Core restart and reset reject an in-flight runtime-store read", async () => {
    ...
    const originalListRunSummaries = handle.store.listRunSummaries.bind(
      handle.store,
    ) as ListRunSummaries;
    ...
    mutableStore.listRunSummaries = (async (
      ...args: Parameters<ListRunSummaries>
    ) => {
      markReadEntered?.();
      await readCanFinish;
    ...
    try {
      ...
      await withTimeout(
        readEntered,
        5000,
        "Timed out waiting for the runtime-store read.",
      );
  ...
  test("Local Core serializes reset against every other maintenance request", async () => {
    ...
    try {
      ...
      await withTimeout(
        runtimeCloseEntered,
        5000,
        "Timed out waiting for reset maintenance to close the runtime.",
      );
  ...
  test("Local Core provider readiness follows the authoritative credential store", async () => {
    ...
    try {
      ...
      assert.equal(
        credentialStatus.credentials.find(
          (entry) => entry.id === "provider.openrouter.default",
        )?.configured,
        true,
      );
      assert.equal(
      ...
      assert.equal(
        serializedRuntimeConfiguration.includes("stored-openrouter-key"),
        false,
      );
      assert.equal(
        serializedRuntimeConfiguration.includes(
          "ambient-openai-key-must-not-count",
        ),
        false,
      );
      assert.equal(
        serializedRuntimeConfiguration.includes(
          "ambient-anthropic-key-must-not-count",
        ),
        false,
      );
      ...
      await client.setCredential("tool.tavily.default", "stored-tavily-key");
      assert.deepEqual(
  ...
  test("Local Core credential mutations are write-only and return sanitized status", async () => {
    ...
    try {
      ...
      assert.equal(
        saved.credentials.find(
          (entry) => entry.id === "tool.visual-crossing.default",
        )?.configured,
        true,
      );
      assert.equal(
        JSON.stringify(saved).includes("visual-crossing-secret"),
        false,
      );
      assert.equal(
      ...
      const deleted = await client.deleteCredential(
        "tool.visual-crossing.default",
      );
  ...
  test("Local Core migrates a legacy plaintext external database URL into credential storage", async () => {
    ...
    await writeFile(
      path.join(settingsDirectory, "local-core-settings.json"),
      JSON.stringify({ databaseMode: "default", databaseUrl: legacyUrl }),
    );
    const credentialStore = new MemoryLocalCoreCredentialStore();
    const server = await startLocalCoreApiServer({
      env: { KESTREL_CORE_HOME: home },
    ...
    try {
      assert.equal(
        await credentialStore.get("data.database.external"),
        legacyUrl,
      );
  ...
  test("Local Core API reports execution unavailable after a failed store restart", async () => {
    ...
    try {
      ...
      await assert.rejects(
        () => client.restart(),
        (error) =>
          error instanceof LocalCoreApiError && error.statusCode === 500,
      );
      const failedStatus = await client.status();
      assert.equal(failedStatus.state, "blocked");
      assert.equal(
        failedStatus.lastError?.code,
        "LOCAL_CORE_EXECUTION_INIT_FAILED",
      );
      assert.match(
        failedStatus.lastError?.message ?? "",
      ...
      await assert.rejects(
        () => client.runs(),
        (error) =>
          error instanceof LocalCoreApiError && error.statusCode === 503,
      );
  ...
  test("Local Core replays durable execution events to the SDK after restart", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-execution-"),
    );
    const start = async () =>
      await startLocalCoreApiServer({
        env: { KESTREL_CORE_HOME: home },
        platform: "darwin",
        coreVersion: "0.6.0",
        idleTimeoutMs: 0,
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          assert.equal(
            terminal.payload.result.assistantText,
            "Execution survived the client boundary.",
          );
      ...
      try {
        ...
        try {
          ...
          const replayed = await withTimeout(
            replay[Symbol.asyncIterator]().next(),
          );
  ...
  test("CLI disconnect leaves a durable Core run available to another client", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-cli-disconnect-"),
    );
    let releaseRun: (() => void) | undefined;
    const runCanFinish = new Promise<void>((resolve) => {
      releaseRun = resolve;
    ...
    try {
      ...
      await withTimeout(
        started,
        5000,
        "Timed out waiting for the CLI run to start.",
      );
      assert.ok(startedEventId);
      unsubscribe();
      await cli.close();
  ...
  test("Local Core API exposes shared workspace and legacy-state endpoints", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-workspaces-"),
    );
    const workspaceRoot = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-workspace-"),
    );
    const server = await startLocalCoreApiServer({
      env: { KESTREL_CORE_HOME: home },
      platform: "darwin",
    ...
    try {
      ...
      const legacy = (await client.legacyState()) as {
        legacyState?: {
          coreHome?: string | undefined;
          entries?: Array<{ name: string; status: string }> | undefined;
        };
      };
      assert.equal(
        legacy.legacyState?.coreHome,
        resolveLocalCorePaths(home).stateRootPath,
      );
      assert.equal(
        legacy.legacyState?.entries?.some(
          (entry) => entry.name === "local_core" && entry.status === "present",
        ),
        true,
      );
    } finally {
  ...
  test("Local Core API owns default shell stores through client-backed adapters", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-stores-"),
    );
    const server = await startLocalCoreApiServer({
      env: { KESTREL_CORE_HOME: home },
      platform: "darwin",
    ...
    try {
      ...
      assert.equal(
        (await new WorkspaceStore(home).load()).workspaces[0]?.workspaceId,
        "ws-core",
      );
      ...
      assert.equal(
        profiles.some((profile) => profile.id === "kestrel"),
        true,
      );
      ...
      assert.equal(
        (await new HistoryStore(home).readTranscript("session-shell"))[0]?.text,
        "hello from Core",
      );
      ...
      assert.equal(
        (await readRuntimeSettings(home)).defaults.minimalMode,
        true,
      );
      ...
      assert.equal(
        (await new KcronStateStore(home).load()).daemon?.pid,
        process.pid,
      );
    } finally {
  ...
  test("Local Core API owns kcron duplicate lease decisions", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-kcron-"),
    );
    const server = await startLocalCoreApiServer({
      env: { KESTREL_CORE_HOME: home },
      platform: "darwin",
    ...
    try {
      ...
      const acquired = (await client.postJson("/v1/kcron/lease/acquire", {
        ownerPid: process.pid,
      })) as { acquired?: boolean };
      ...
      await client.postJson("/v1/kcron/lease/release", {
        ownerPid: process.pid,
      });
  ...
  test("Local Core registers a Core-owned Desktop execution profile resolved from model policy", async () => {
      ...
      executionRuntimeFactory: (profile) => {
        ...
        return {
          async runTurn(turn) {
            return {
              assistantText:
                "Local Core accepted the registered Desktop profile.",
              finalizedPayload: null,
              output: {
                status: "COMPLETED" as const,
                sessionId: turn.sessionId,
    ...
    try {
      ...
      assert.equal(
        initialRuntimeConfiguration.modelPolicy.provider,
        "openrouter",
      );
      ...
      assert.equal(initial.resolvedProfile.presetId, "desktop_safe_local");
      assert.equal(initial.resolvedProfile.modelProvider, "openrouter");
      assert.equal(
        initial.modelReadiness.registration.providerId,
        initial.resolvedProfile.modelProvider,
      );
      assert.equal(
        initial.modelReadiness.registration.modelId,
        initial.resolvedProfile.model,
      );
      assert.equal(initial.modelReadiness.qualification, "pending");
      assert.deepEqual(initial.modelReadiness.eligibleRoles, []);
      ...
      assert.equal(
        updatedRuntimeConfiguration.environmentOptionsMode,
        "inherit",
      );
      assert.equal(updatedRuntimeConfiguration.modelPolicy.provider, "ollama");
      assert.equal(
        updatedRuntimeConfiguration.modelPolicy.model,
        "llama3.2:latest",
      );

      const refreshedReadiness = await client.refreshDesktopModelReadiness();
      assert.equal(refreshedReadiness.qualification, "qualified");
      assert.equal(
        refreshedReadiness.registration.capabilities.localSchemaValidation
          .state,
        "qualified",
      );
      assert.equal(
      ...
      assert.equal(
        resolved.resolvedProfile.approvalPolicyPackId,
        "isolated_code",
      );
      ...
      const listedDesktop = listed.find(
      ...
      assert.equal(
        listed.filter((profile) => profile.id === resolved.profileId).length,
        1,
      );
  ...
  test("Local Core API mirrors Desktop UI state without overwriting TUI state", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-desktop-ui-"),
    );
    const server = await startLocalCoreApiServer({
      env: { KESTREL_CORE_HOME: home },
      platform: "darwin",
    ...
    try {
      ...
      assert.equal(
        (await client.getDesktopUiState())?.entries["kchat:web:theme-mode"],
        "light",
      );

      const tuiState = (await client.getJson("/v1/ui-state")) as {
        state?: unknown;
      };
  ...
  test("Local Core API restart swaps execution ownership across blocked and healthy stores", async () => {
    ...
    try {
      ...
      assert.equal(
        missingUrlStatus.lastError?.code,
  ...
  test("Local Core API owns Desktop project runs and streams changes", async () => {
    const home = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-desktop-runs-"),
    );
    const project = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-core-api-project-"),
    );
    await writeFile(
    ...
    try {
      ...
      try {
        const launcher = await client.readDesktopProjectLauncher({
          projectPath: project,
        });
        assert.equal(launcher?.packageManager, "npm");
        assert.equal(
          launcher?.scripts.some((script) => script.name === "dev"),
          true,
        );

        const run = await client.startDesktopProjectRun({
          projectPath: project,
          scriptName: "dev",
        });
        ...
        await waitFor(() =>
          events.some((event) =>
            event.runs.some((entry) => entry.runId === run.runId),
          ),
        );
        await waitFor(async () => {
          ...
          events.some((event) =>
            event.runs.some(
              (entry) =>
                entry.runId === run.runId &&
                entry.outputTail?.some(
                  (line) => line.line === "http://127.0.0.1:4123",
                ) === true,
            ),
          ),
        );
        ...
        const reset = await client.resetRuntimeStore();
        assert.equal(reset.status.state, "healthy");
        const preserved = (await client.listDesktopProjectRuns()).find(
        ...
        await waitFor(() =>
          events.some((event) =>
            event.runs.some(
              (entry) =>
                entry.runId === run.runId && entry.status === "stopped",
            ),
          ),
        );
      } finally {
...
async function destroySlowRequest(
  requestToDestroy: ClientRequest,
): Promise<void> {
