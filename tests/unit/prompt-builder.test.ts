...
test("build-mode deliberator prompt stays compact and generic", () => {
  ...
  assert.match(act, /likely target/u);
  assert.match(act, /intended candidate change/u);
  ...
  assert.match(act, /Once the likely target, pattern, and validation path are known/u);
  assert.match(act, /source, config, artifact, UI, API, command, or documentation change/u);
  assert.match(act, /If the task names a required file, output, command, API\/function call, user-facing artifact, format, or deliverable/u);
  assert.match(act, /create or update that concrete deliverable as soon as its intended shape is understood/u);
  ...
  assert.match(act, /When exec_command is available/u);
  assert.match(act, /It is for starting work, not for typing into a running program/u);
  assert.match(act, /Continue it by passing that `sessionId`/u);
  assert.match(act, /include the newline a terminal user would press/u);
  assert.match(act, /Do not wrap interactive input in a new shell command/u);
  ...
  assert.match(act, /Before finalizing an artifact task/u);
