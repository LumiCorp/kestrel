...
import { normalizeToolActionInput, sanitizeToolInputForSchema } from "./normalizeToolInput.js";
import { RunCancelledError, RuntimeFailure, createRuntimeFailure } from "../../src/runtime/RuntimeFailure.js";
import { buildAgentToolFailureResult, buildAgentToolSuccessResult } from "../toolResult.js";
...
export class UnifiedToolRegistry implements ToolGateway, ToolRegistry {
  ...
  async preRun(_context: ToolGatewayPreRunContext): Promise<void> {
  ...
  getModelTools(options: ToolRegistryListOptions = {}): ModelToolSpec[] {
    ...
    for (const name of allowlist) {
      ...
      const mcpTool = this.mcpStatus.tools.find((tool) => tool.namespacedToolName === name);
  ...
  getCapabilityManifest(options: ToolRegistryListOptions = {}): CapabilityManifestItem[] {
  ...
  resolveAvailableAllowlist(names: string[]): string[] {
    const available = new Set(this.listAvailableToolNames());
    return [...new Set(names)].filter((name) =>
      available.has(name) || this.isRuntimeBuiltInToolName(name)
    );
  }
  ...
  async validateInput(
    ...
    if (scopedContext.allowlist.has(name) === false) {
      throw createRuntimeFailure("TOOL_LOOKUP_FAILED", `Tool '${name}' is not allowlisted.`, {
        subsystem: "tooling",
        toolName: name,
    ...
    if (
      ...
      throw createRuntimeFailure("TOOL_DISABLED_FOR_PROFILE", `Tool '${name}' is disabled for this profile.`, {
        subsystem: "tooling",
        toolName: name,
        classification: "policy",
        recoverable: true,
    ...
    const normalizedInput = recordInput !== undefined
      ...
      : input;
    const schema = this.resolveInputSchema(name);
    ...
    if (valid !== true) {
      if (this.builtInToolSpecs.has(name)) {
        throw createBuiltInSchemaValidationError(name, schemaSanitizedInput, validator.errors ?? []);
      }
      throw new RuntimeFailure("TOOL_INPUT_SCHEMA_FAILED", `Tool '${name}' input failed schema validation.`, {
        toolName: name,
  ...
  async call(
    ...
    if (this.builtInToolSpecs.has(name)) {
      ...
      if (builtIn === undefined) {
        throw createRuntimeFailure("TOOL_LOOKUP_FAILED", `Tool '${name}' is not available.`, {
          subsystem: "tooling",
          toolName: name,
          classification: "configuration",
          recoverable: false,
    ...
    const mcpTool = this.resolveExposedMcpTool(name);
    if (mcpTool !== undefined) {
      const startedAt = new Date().toISOString();
      try {
        const output = await this.mcpManager.callTool(name, validatedInput);
        this.throwIfAborted(options.signal);
        ...
        if (error instanceof RuntimeFailure && error.details?.recoverable === false) {
          throw error;
        }
  ...
  private resolveInputSchema(name: string): Record<string, unknown> {
    const builtIn = this.builtInToolSpecs.get(name);
    if (builtIn !== undefined) {
  ...
  private getValidator(name: string, schema: Record<string, unknown>): ValidateFunction {
  ...
  private listAvailableToolNames(): string[] {
    ...
    for (const tool of this.mcpStatus.tools) {
  ...
  private resolveExposedMcpTool(name: string) {
    const tool = this.mcpStatus.tools.find((candidate) => candidate.namespacedToolName === name);
...
function createBuiltInSchemaValidationError(
  ...
  const field = firstError === undefined ? "input" : readAjvErrorField(firstError);
  const expected = firstError === undefined ? "input satisfying tool schema" : readAjvErrorExpectation(firstError);
  const invalidValues = firstError === undefined ? [] : readAjvErrorInvalidValues(input, firstError);
...
function resolveScopedRunContext(
  ...
  const workspaceRoot =
    typeof workspace?.workspaceRoot === "string" && workspace.workspaceRoot.trim().length > 0
      ? workspace.workspaceRoot
      : undefined;
  const workspaceToolContext = {
    ...(typeof workspace?.appRoot === "string" && workspace.appRoot.trim().length > 0
      ? { appRoot: workspace.appRoot.trim() }
      : {}),
    ...(typeof workspace?.packageManager === "string" && workspace.packageManager.trim().length > 0
  ...
  const executionTicket = readKestrelOneExecutionTicket(payload);
  ...
  const sourceWriteGuardAllowedWriteRoots = resolveDevShellSourceWriteAllowedWriteRoots(
    workspaceRoot,
  ...
  const scopedBaseContext: SharedToolContext =
    {
      ...
      executionTicket !== undefined
        ...
        : {}),
      ...(Object.keys(workspaceToolContext).length > 0 ? { workspace: workspaceToolContext } : {}),
      ...(devShellSourceWriteApprovalGrants.length > 0 || trustedManagedWorktree || sourceWriteAuthority !== undefined
        ? {
            devShell: {
...
function readInteractionMode(payload: unknown): "chat" | "plan" | "build" | undefined {
  ...
  if (fromMetadata === "chat" || fromMetadata === "plan" || fromMetadata === "build") {
    return fromMetadata;
  }
  const orchestration = asRecord(asRecord(payload)?.orchestration);
  const fromOrchestration = orchestration?.interactionMode;
  return fromOrchestration === "chat" || fromOrchestration === "plan" || fromOrchestration === "build"
    ? fromOrchestration
    : undefined;
}
...
function resolveRuntimeToolRunContext(
  ...
  const threadId = asNonEmptyString(orchestration?.threadId) ?? asNonEmptyString(metadata?.threadId);
  const activeTaskId =
  ...
  const delegationId =
    asNonEmptyString(orchestration?.delegationId) ?? asNonEmptyString(metadata?.delegationId);
...
function hasTrustedManagedWorktreeBinding(runId: string, state: unknown, payload: unknown, sessionId: string): boolean {
  const stateRecord = asRecord(state);
  const binding =
    asRecord(asRecord(asRecord(stateRecord?.agent)?.exec)?.managedWorktreeBinding) ??
    asRecord(asRecord(asRecord(stateRecord?.react)?.exec)?.managedWorktreeBinding);
  ...
  return (
    ...
    asNonEmptyString(workspace.workspaceRoot) === asNonEmptyString(binding.worktreeRoot) &&
...
function readDevShellSourceWriteApprovalGrants(payload: unknown) {
  ...
  return grants.flatMap((item) => {
    ...
    const writablePaths = Array.isArray(record?.writablePaths)
      ? record.writablePaths.filter((value): value is string => typeof value === "string" && value.trim().length > 0)
      : [];
    if (grantId === undefined || command === undefined || writablePaths.length === 0) {
      return [];
    }
    return [{
      grantId,
      command,
      writablePaths,
      ...(asNonEmptyString(record?.cwd) !== undefined ? { cwd: asNonEmptyString(record?.cwd) } : {}),
      ...(asNonEmptyString(record?.expiresAt) !== undefined ? { expiresAt: asNonEmptyString(record?.expiresAt) } : {}),
    }];
  });
}
...
function readKestrelOneExecutionTicket(payload: unknown): string | undefined {
  const clientCapabilities = asRecord(asRecord(payload)?.clientCapabilities);
  const kestrelOne = asRecord(clientCapabilities?.kestrelOne);
  return asNonEmptyString(kestrelOne?.executionTicket);
}
