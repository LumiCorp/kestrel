...
export function buildOpenRouterHttpRequestV2(
  ...
  if (
    endpoint === "responses" &&
    (request.reasoning?.continuation?.length ?? 0) > 0
  ) {
    throw createOpenRouterBadResponseError(
  ...
  const payload =
    endpoint === "chat"
      ? buildChatBody(request, model, route, eligibleEndpointIds)
      : buildResponsesBody(request, model, route, eligibleEndpointIds);
  return {
    endpoint,
    model,
    path:
      endpoint === "chat" ? "/api/v1/chat/completions" : "/api/v1/responses",
    body: payload.body,
...
function requireEndpoint(
  ...
  if (
    route.routing.kind === "fixed" &&
    route.routing.allowedEndpointIds.length !== 1
  ) {
    throw createOpenRouterBadResponseError(
...
function eligibleEndpointIdsFor(
  route: OpenRouterQualifiedRouteEvidence,
  ...
  const eligible = route.routing.allowedEndpointIds.filter((endpointId) => {
    const endpoint = capabilitiesById.get(endpointId);
    return (
      endpoint !== undefined &&
  ...
  if (eligible.length === 0) {
    throw createOpenRouterBadResponseError(
      `OpenRouter qualified route '${route.routing.policyId}' has no eligible provider endpoint for required parameters '${requiredParameters.join(", ")}'.`,
    );
  }
  return eligible;
}
...
function applyCommonParameters(
  ...
  if (typeof options?.maxTokens === "number") {
    body[endpoint === "chat" ? "max_tokens" : "max_output_tokens"] =
      options.maxTokens;
  }
...
function toolChoice(
  ...
  if (requirements.toolName === undefined) {
    throw createOpenRouterBadResponseError(
      "Named OpenRouter tool choice requires its exact tool name.",
    );
  }
...
function chatStructuredOutput(request: ModelRequestV2) {
  ...
  if (request.requirements.output.kind === "json_object") {
    return {
      ...
      structuredOutput: { mode: "json_object" as const },
    };
  }
  const schemaName =
    request.requirements.output.schemaName ?? "kestrel_response";
...
function responsesStructuredOutput(request: ModelRequestV2) {
  ...
  const jsonSchema = compiled.responseFormat.json_schema;
  return {
    value: {
      type: "json_schema",
...
function requiredOpenRouterParameters(
  ...
  if (request.reasoning?.continuation?.length)
    required.add("reasoning_details");
...
function applyChatContinuation(
  messages: Array<Record<string, unknown>>,
  request: ModelRequestV2,
): void {
  const continuation = request.reasoning?.continuation ?? [];
  if (continuation.length === 0) return;
  const details = continuation.filter(
    (entry) =>
      entry.provider === "openrouter" && entry.kind === "reasoning_details",
  );
  if (details.length !== continuation.length) {
    throw createOpenRouterBadResponseError(
      "OpenRouter chat cannot encode a continuation from another provider or kind.",
    );
  }
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    if (messages[index]?.role === "assistant") {
      messages[index] = {
        ...messages[index],
        reasoning_details: details.map((entry) => entry.value),
      };
      return;
    }
  }
  throw createOpenRouterBadResponseError(
    "OpenRouter continuation requires the preceding assistant message.",
  );
}
...
function mapResponsesMessage(
  message: ModelMessage,
): Array<Record<string, unknown>> {
  if (message.role === "assistant" && message.toolCalls?.length) {
    return [
      ...(contentText(message.content).trim()
        ? [{ role: "assistant", content: mapResponsesContent(message.content) }]
        : []),
  ...
  if (message.role === "tool") {
    if (message.toolCallId === undefined)
      throw createOpenRouterBadResponseError(
        "OpenRouter Responses tool output requires a call ID.",
      );
    return [
      {
  ...
  return [
    { role: message.role, content: mapResponsesContent(message.content) },
  ];
}
...
function mapChatResponse<TOutput>(
  payload: unknown,
...
function mapResponsesResponse<TOutput>(
  payload: unknown,
  context: Parameters<typeof mapOpenRouterResponseV2>[1],
): ModelResponseV2<TOutput> {
  const root = record(payload);
  ...
  const text =
    ...
    (outputItems.flatMap(outputText).join("\n") || undefined);
...
function responseV2<TOutput>(
  value: Omit<ModelResponseV2<TOutput>, "version" | "validation">,
): ModelResponseV2<TOutput> {
  return {
    ...value,
    version: MODEL_RESPONSE_V2_VERSION,
    validation:
      value.terminal.state === "completed"
        ? { state: "not_requested" }
        : {
            state: "failed",
            failureCode: `MODEL_${value.terminal.state.toUpperCase()}_RESPONSE`,
          },
  };
}
...
function malformedResponse<TOutput>(
  ...
  const event =
    context.endpoint === "chat"
      ? (string(root?.__openRouterTerminalEvent) ??
        (string(record(array(root?.choices)[0])?.finish_reason) !== undefined
          ? "chat.completion"
          : undefined))
      : (string(root?.__openRouterTerminalEvent) ??
        (string(root?.status) !== undefined
          ? "response.completed"
          : undefined));
  return {
    version: MODEL_RESPONSE_V2_VERSION,
...
function chatTerminal(
  ...
  if (finish === "stop" || finish === "tool_calls")
    return {
      ...
      providerTerminalEvent: "chat.completion",
    };
  if (finish === "length")
    return {
      state: "truncated",
  ...
  if (streamEvent === "[DONE]")
    return {
      ...
      providerTerminalEvent: streamEvent,
    };
  return { state: "malformed", visibleOutputStarted: false };
}

function responsesTerminal(
  root: Record<string, unknown> | undefined,
): ModelResponseV2["terminal"] {
  ...
  if (event === "response.completed" || status === "completed")
    return {
      state: "completed",
      visibleOutputStarted: event !== undefined,
      providerTerminalEvent: event ?? "response.completed",
    };
  if (status === "incomplete")
    return {
      state: "incomplete",
      visibleOutputStarted: false,
      providerTerminalEvent: event,
    };
    ...
    ...(context.structuredOutput !== undefined
      ? {
          structuredOutput: {
            ...
            outcome:
              source === "provider"
                ? ("provider_parsed" as const)
                : source === "text"
                  ? ("text_fallback_parsed" as const)
            ...
            ...(context.structuredOutput.schemaName !== undefined
              ...
              : {}),
            ...(context.structuredOutput.compilerDiagnostics !== undefined
              ? {
                  compilerDiagnostics:
                    context.structuredOutput.compilerDiagnostics,
                }
              : {}),
          },
        }
...
function chatToolIntents(value: unknown): ModelToolIntent[] {
  return array(value).map((entry) => {
    ...
    const input = exactObjectJson(
      fn?.arguments,
      "OpenRouter Chat tool arguments",
    );
    if (name === undefined || id === undefined)
      throw createOpenRouterBadResponseError(
        "OpenRouter Chat returned a tool call without an ID or name.",
      );
...
function responsesToolIntents(value: unknown[]): ModelToolIntent[] {
  return value
    .filter((entry) => record(entry)?.type === "function_call")
    .map((entry) => {
      const call = record(entry)!;
      ...
      if (name === undefined || id === undefined)
        throw createOpenRouterBadResponseError(
          "OpenRouter Responses returned a function call without an ID or name.",
        );
      return {
        id,
        name,
        input: exactObjectJson(
          call.arguments,
          "OpenRouter Responses function arguments",
        ),
      };
    });
}
...
function chatReasoning(
  ...
  for (const detail of details) {
    ...
    if (item?.type === "reasoning.text" && string(item.text) !== undefined) {
      visible.push({
        format: "provider_reasoning_text",
        text: string(item.text)!,
      });
    } else if (
      item?.type === "reasoning.summary" &&
      string(item.summary) !== undefined
    ) {
      visible.push({ format: "summary", text: string(item.summary)! });
    }
  }
  return visible.length || details.length
    ? {
        visible,
        continuation: details.length
          ? [
              {
                provider: "openrouter",
...
function responsesReasoning(
  ...
  const visible = details.flatMap((entry) => {
    ...
    return summary === undefined
      ? []
      : [{ format: "provider_reasoning_text" as const, text: summary }];
  });
...
function outputText(value: unknown): string[] {
  ...
  if (item?.type === "output_text" && string(item.text) !== undefined)
    return [string(item.text)!];
  return array(item?.content).flatMap(outputText);
}

function textFromChat(
  message: Record<string, unknown> | undefined,
): string | undefined {
  return (
...
function exactObjectJson(
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
    throw createOpenRouterBadResponseError(`${label} were malformed.`);
...
function mapUsage(
  ...
  const input = number(
    endpoint === "chat" ? value.prompt_tokens : value.input_tokens,
  );
  const output = number(
    endpoint === "chat" ? value.completion_tokens : value.output_tokens,
  );
