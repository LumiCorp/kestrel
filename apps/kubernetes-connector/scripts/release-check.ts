...
assert.match(resources, /maxUnavailable: 1/u);
assert.match(resources, /maxSurge: 1/u);
assert.match(resources, /kind: PodDisruptionBudget[\s\S]*minAvailable:/u);
assert.match(resources, /runAsNonRoot: true/u);
assert.match(resources, /automountServiceAccountToken: false/u);
assert.match(resources, /readOnlyRootFilesystem: true/u);
assert.match(resources, /allowPrivilegeEscalation: false/u);
assert.match(resources, /pod-security\.kubernetes\.io\/enforce: restricted/u);
assert.match(resources, /serviceAccountToken:/u);
assert.match(resources, /expirationSeconds: 3600/u);
assert.match(resources, /resourceNames: \["kestrel-environment-manager"\]/u);
assert.doesNotMatch(resources, /image:\s+[^\n]*:latest/u);
