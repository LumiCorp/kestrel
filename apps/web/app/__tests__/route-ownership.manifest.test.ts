...
function readRouteGuardSource(file: string) {
  ...
  const sharedRoute = source.match(
    /from "@\/(app\/api\/mobile\/v1\/.+\/route)"/u,
  )?.[1];
  return sharedRoute
    ? `${source}\n${readAppFile(`${sharedRoute}.ts`)}`
    : source;
}
  ...
  () => {
    for (const entry of KESTREL_ONE_ROUTE_OWNERSHIP_MANIFEST) {
      ...
      if (entry.access === "admin") {
        assert.match(
          source,
          /\brequire(?:Admin(?:Organization)?|OrganizationAdmin|OrganizationOwner)\b/,
