...
import { authorizeKubernetesConnector, proveKubernetesQualificationEndpoint } from "@/lib/environments/kubernetes-connector";
...
export async function POST(request: Request, context: { params: Promise<{ connectionId: string }> }) {
  try {
    ...
    return NextResponse.json(await proveKubernetesQualificationEndpoint({ authorization, value: JSON.parse(bodyText) }));
