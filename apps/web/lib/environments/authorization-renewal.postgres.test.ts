...
test("execution authorization renews active authority and denies stale authority", async (context) => {
  ...
  await sql.begin(async (tx) => {
    ...
    await tx`INSERT INTO "environment_workspaces" ("id", "organization_id", "environment_id", "personal_owner_user_id", "created_by_user_id", "name", "kind", "status", "fly_machine_id", "fly_volume_id", "runtime_image") VALUES (${ids.workspace}, ${ids.organization}, ${ids.environment}, ${ids.user}, ${ids.user}, 'Renewal Workspace', 'scratch', 'ready', ${`machine-${suffix}`}, ${`volume-${suffix}`}, 'registry.example/workspace@sha256:test')`;
  });
