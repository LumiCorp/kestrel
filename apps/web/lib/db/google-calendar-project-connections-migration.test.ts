...
test("Google Calendar migration installs project user gates and capability policy", async () => {
...
test("Google Calendar migration never adds Gmail or Drive scopes", async () => {
  const migration = await readFile(migrationUrl, "utf8");
