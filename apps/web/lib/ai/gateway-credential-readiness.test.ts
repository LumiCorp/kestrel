...
contractTest(
  ...
  () => {
    assert.deepEqual(
      getGatewayCredentialAuthorityHealth({ NODE_ENV: "test" }),
      {
        ok: false,
  ...
  () => {
    ...
    assert.deepEqual(
      getGatewayCredentialStorageHealth(
        ...
        {
          NODE_ENV: "test",
          KESTREL_GATEWAY_CREDENTIAL_ACTIVE_KEY_ID: "replacement",
          KESTREL_GATEWAY_CREDENTIAL_KEYS: JSON.stringify({
            replacement: Buffer.alloc(32, 9).toString("base64"),
          }),
        },
      ),
      { ok: false, code: "GATEWAY_CREDENTIAL_KEY_UNKNOWN" },
    );
  },
);
