...
export const ENVIRONMENT_RUNTIME_PROMOTION_AUDIENCE =
  "kestrel-environment-runtime-promotion";
export const ENVIRONMENT_RUNTIME_WORKFLOW_REF =
  "LumiCorp/kestrel/.github/workflows/production-fly.yml@refs/heads/production";
const GITHUB_REPOSITORY = "LumiCorp/kestrel";
const GITHUB_REF = "refs/heads/production";
...
const jwtClaimsSchema = z.object({
  ...
  workflow_ref: z.literal(ENVIRONMENT_RUNTIME_WORKFLOW_REF),
  workflow_sha: z.string().regex(/^[a-f0-9]{40}$/u),
...
export async function verifyEnvironmentRuntimeWorkflowToken(input: {
  token: string;
  ...
  if (parts.length !== 3) throw new Error("GitHub Actions OIDC token is invalid.");
  ...
  if (!audiences.includes(ENVIRONMENT_RUNTIME_PROMOTION_AUDIENCE)) {
    throw new Error("GitHub Actions OIDC token has the wrong audience.");
  }
  if (
    claims.sha !== input.expectedWorkflowSha ||
    claims.workflow_sha !== input.expectedWorkflowSha ||
    claims.run_id !== input.expectedRunId ||
    Number(claims.run_attempt) !== input.expectedRunAttempt
  ) {
    throw new Error("GitHub Actions OIDC token has the wrong workflow run identity.");
  }
  ...
  const signingKey = jwks.keys.find((candidate) => candidate.kid === header.kid);
  if (!signingKey) throw new Error("GitHub Actions OIDC signing key is unknown.");
