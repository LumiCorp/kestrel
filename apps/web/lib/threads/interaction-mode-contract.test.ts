...
const composer = fs.readFileSync(
  path.join(root, "components/chatbot/multimodal-input.tsx"),
  "utf8",
);
...
test("task interaction mode is canonical across API, composer, turns, and runtime switches", () => {
  ...
  assert.match(chat, /executeKestrelOneComposerModeSelection/u);
  assert.match(composer, /modeDisabled=\{composerBlockedBySetup\}/u);
  assert.doesNotMatch(composer, /modeDisabled=\{[^}]*pendingInteraction/u);
