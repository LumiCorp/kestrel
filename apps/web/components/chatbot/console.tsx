...
export function Console({ consoleOutputs, setConsoleOutputs }: ConsoleProps) {
  ...
  return consoleOutputs.length > 0 ? (
    <>
      ...
      <div
        className={cn(
          "fixed bottom-0 z-40 flex w-full flex-col overflow-x-hidden overflow-y-scroll border-t bg-code text-code-foreground",
          {
        ...
        style={{ height }}
      >
        <div className="sticky top-0 z-50 flex h-fit w-full flex-row items-center justify-between border-b bg-muted px-2 py-1">
        ...
        <div>
          {consoleOutputs.map((consoleOutput, index) => (
            <div
              className="flex flex-row border-b bg-code px-4 py-2 font-mono text-code-foreground text-sm"
            ...
            >
              ...
              {["in_progress", "loading_packages"].includes(
                ...
                <div className="flex w-full flex-col gap-2 overflow-x-scroll text-code-foreground">
                  {consoleOutput.contents.map((content) =>
                    content.type === "image" ? (
