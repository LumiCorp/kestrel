...
export type GatewayAdminSurface = "connections" | "models";
...
function GatewayProviderList({
  ...
  gatewayCollectionState: ReturnType<typeof getGatewayCollectionState>;
  ...
  return (
    <SettingsSection
      description="Choose the provider whose catalog or connection you want to manage."
      title="Providers"
    >
      {gatewayCollectionState === "loading" ? (
        ...
        <div className="border-y py-5 text-muted-foreground text-sm">
          ...
          >
            Add a provider from Connections.
          </Link>
        </div>
      ) : (
        <SettingsRows>
          {gateways.map((bundle) => {
            ...
            return (
              <button
                ...
                onClick={() => onSelect(bundle.gateway.id)}
                type="button"
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-muted-foreground text-xs">
                    {overview.approvedCount} approved · {overview.defaultCount}{" "}
                    default
                  </div>
                </div>
                <div className="min-w-0">
                  <SettingsStatusSummary
                    detail={
                      bundle.gateway.updatedAt
                        ? `Updated ${new Date(bundle.gateway.updatedAt).toLocaleDateString()}`
                        : undefined
                    }
