...
function serializeCalendarDay(date: Date) {
...
function Calendar({
  ...
  const formatMonthLabel = React.useCallback(
    (date: Date) => {
      const month = date.getMonth() as Parameters<
        NonNullable<NonNullable<typeof props.locale>["localize"]>["month"]
      >[0];
      const localizedLabel = props.locale?.localize?.month(month, {
        width: "abbreviated",
        context: "formatting",
      });

      return localizedLabel || format(date, "MMM");
    },
    [props.locale]
  );
  ...
  return (
    <DayPicker
      ...
      formatters={{
        formatMonthDropdown: formatMonthLabel,
        ...formatters,
      }}
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      data-day={serializeCalendarDay(day.date)}
