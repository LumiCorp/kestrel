...
contractTest(
  ...
  () => {
    ...
    assert.equal(
      settings.modelConfigurations[0]?.revisions[0]?.policy.provider,
      "anthropic",
    );
    assert.equal(
      settings.modelConfigurations[0]?.revisions[0]?.policy.model,
      "claude-sonnet-4-5",
    );
  },
);
...
contractTest(
  ...
  async () => {
    const tempDir = await mkdtemp(
      path.join(os.tmpdir(), "kestrel-desktop-settings-"),
    );
    const settings = await readDesktopSettings(
      path.join(tempDir, "desktop-settings.json"),
    );
...
contractTest(
  ...
  () => {
      ...
      credentials: [
        {
          ...
          credentialId: "mcp.candidate.bearer.token" as const,
          envKey: "KESTREL_MCP_CANDIDATE_TOKEN",
          configured: true,
        },
      ],
    };
    const settings = normalizeDesktopSettings({
      mcpServers: [
        { ...base, url: "https://lookalike.example.test/mcp" },
        {
          ...base,
          id: "official",
          ...
          credentials: [
            {
              ...base.credentials[0]!,
              credentialId: "mcp.official.bearer.token" as const,
            },
          ],
        },
      ],
    });
    assert.equal(settings.mcpServers[0]?.appId, undefined);
    assert.equal(settings.mcpServers[1]?.appId, "linear");
  },
);
...
contractTest(
  ...
  async () => {
    ...
    assert.equal(
      saved.providerSelectionCompletedAt,
    ...
    assert.match(
      raw,
      /"providerSelectionCompletedAt": "2026-04-18T00:00:00.000Z"/u,
    );
...
contractTest(
  ...
  () => {
    ...
    assert.equal(
      env.DATABASE_URL,
      "postgres://kestrel:kestrel@localhost:55499/kestrel",
    );
...
contractTest(
  ...
  () => {
    ...
    assert.equal(
      profile.mcpServers?.[0]?.toolMetadata?.search?.approvalMode,
      "ask",
    );
    assert.deepEqual(
      profile.mcpServers?.[0]?.toolMetadata?.search?.allowedInteractionModes,
      ["build"],
    );
...
contractTest(
  ...
  () => {
      ...
      mcpServers: [
        {
          ...
          oauthCredentialPrefix: "mcp.standard.notion" as const,
          enabled: true,
          source: "Kestrel Desktop",
          sourceKind: "desktop-managed" as const,
          tools: [{ name: "search" }],
    ...
    assert.equal(
      profile.mcpServers?.[0]?.oauthCredentialPrefix,
      "mcp.standard.notion",
    );
  },
);
...
contractTest(
  ...
  () => {
      ...
      mcpServers: [
        {
          ...
          capabilityPacks: ["search", "messages", "search", "unknown"],
          enabled: true,
          source: "Kestrel Desktop",
    ...
    assert.equal(settings.mcpServers[0]?.appId, undefined);
    assert.equal(settings.mcpServers[0]?.oauthCredentialPrefix, undefined);
    assert.equal(settings.mcpServers[0]?.capabilityPacks, undefined);
      ...
      mcpServers: [
        {
          ...
          url: "https://mcp.slack.com/mcp",
          oauthCredentialPrefix: "mcp.standard.slack",
          capabilityPacks: ["search", "messages"],
          enabled: true,
          source: "Kestrel Desktop",
          sourceKind: "desktop-managed",
          tools: [{ name: "search" }],
    ...
    assert.equal(valid.mcpServers[0]?.appId, "slack");
    assert.deepEqual(valid.mcpServers[0]?.capabilityPacks, [
      "search",
      "messages",
    ]);
  },
...
contractTest(
  ...
  () => {
    ...
    assert.equal(valid.mcpServers[0]?.appId, "vercel");
    assert.deepEqual(valid.mcpServers[0]?.tools?.map((tool) => tool.name), [
      "list_projects",
      "get_project",
    ]);

    const missingRequiredTool = normalizeDesktopSettings({
      mcpServers: [
        {
          ...valid.mcpServers[0]!,
    ...
    assert.equal(missingRequiredTool.mcpServers[0]?.appId, undefined);
    assert.equal(
      missingRequiredTool.mcpServers[0]?.oauthCredentialPrefix,
      undefined,
    );
    assert.equal(missingRequiredTool.mcpServers[0]?.capabilityPacks, undefined);
  },
);
...
contractTest(
  ...
  () => {
    const valid = normalizeDesktopSettings({
      mcpServers: [{
        id: "standard.microsoft_365",
        appId: "microsoft_365",
        name: "Microsoft 365",
        transport: "http",
        url: "https://graph.microsoft.com/v1.0",
        oauthCredentialPrefix: "mcp.standard.microsoft_365",
        capabilityPacks: ["outlook"],
        enabled: true,
        source: "Kestrel Desktop",
        sourceKind: "desktop-managed",
        tools: [
    ...
    assert.equal(valid.mcpServers[0]?.appId, "microsoft_365");
    assert.deepEqual(valid.mcpServers[0]?.tools?.map((tool) => tool.name), [
    ...
    const incomplete = normalizeDesktopSettings({
      mcpServers: [{ ...valid.mcpServers[0]!, tools: [{ name: "microsoft_365.list_mail" }] }],
    });
  ...
  () => {
    ...
    const result = buildDesktopExecutionProfile(
      ...
      settings,
      {
        ...
        apps: [{ id: "linear", contractVersion: 1 }],
      },
    );

    assert.equal("profile" in result, true);
    if (!("profile" in result)) return;
    assert.equal(result.profile.mcpServers?.[0]?.id, "standard.linear");
    assert.equal(
      result.profile.toolAllowlist?.includes(
        "mcp.standard.linear.create_issue",
      ),
      true,
    );
    assert.equal(
      result.profile.toolAllowlist?.includes("mcp.standard.linear.list_issues"),
      true,
    );
  },
);
...
contractTest(
  ...
  () => {
    ...
    assert.deepEqual(result, {
      missingApp: { id: "linear", contractVersion: 1 },
    });
  },
);
...
contractTest(
  ...
  () => {
      ...
      mcpServers: [
        {
          ...
          source: "Kestrel Desktop",
          sourceKind: "desktop-managed" as const,
          oauthCredentialPrefix: "mcp.standard.microsoft_365" as const,
          capabilityPacks: ["outlook"],
          tools: [
            { name: "microsoft_365.list_mail" },
            { name: "microsoft_365.send_mail" },
    ...
    const result = buildDesktopExecutionProfile(
      ...
      {
        modelConfiguration: { id: "desktop-default", revision: 1 },
        apps: [{ id: "microsoft_365", contractVersion: 1 }],
      },
    );
    assert.equal("profile" in result, true);
    if (!("profile" in result)) return;
    assert.deepEqual(result.profile.mcpServers, []);
    assert.ok(result.profile.toolAllowlist?.includes("microsoft_365.list_mail"));
    assert.equal(
      result.profile.toolAllowlist?.some((name) => name.startsWith("mcp.standard.microsoft_365.")),
      false,
    );
  },
);
