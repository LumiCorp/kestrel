...
export function SettingsWorkspace({
  ...
  const readinessSummary = useMemo(
    () => summarizeReadiness(view?.capabilities ?? []),
    [view],
  );
    ...
    <main className="surface-pane settings-surface" id="app-main">
      <header className="surface-header">
        <div>
          ...
          <h1>Settings</h1>
          <p>
            Understand what Kestrel can use, what is ready, and what needs
            setup.
          </p>
        </div>
        <button
      ...
      {view !== undefined ? (
        <>
          <div className="settings-authority-note">
            ...
            <span>
              Credentials are{" "}
              {view.credentialStore.available
                ? "stored securely by Local Core"
                : "unavailable on this system"}{" "}
              and are never returned to this screen.
            </span>
          </div>
          <div
            className="capability-summary"
            aria-label="Capability readiness summary"
          >
            <span>
            ...
            <span>
              ...
              <strong>{readinessSummary.attention}</strong> need attention
            </span>
            <span>
              <Circle size={15} aria-hidden="true" />
              <strong>{readinessSummary.inactive}</strong> inactive or optional
            </span>
          </div>
          <p className="capability-summary-time">
            Readiness last checked {new Date(view.refreshedAt).toLocaleString()}
            .
          </p>
          {attentionCapabilities.length > 0 ? (
            ...
            >
              <div className="capability-attention-heading">
                <div>
                  ...
                  <h2 id="capability-attention-title">
                ...
                <p>
                  These are the current setup or recovery blockers. Resolve them
                  here, then refresh readiness to confirm the effective Desktop
                  state.
                </p>
              </div>
            ...
            <section
              className="capability-attention-queue capability-attention-clear"
              aria-label="Capability readiness"
            >
              <CheckCircle2 size={18} aria-hidden="true" />
              <p>
                <strong>No setup blockers.</strong> Enabled Desktop capabilities
                are ready to use.
              </p>
            </section>
          )}
...
function summarizeReadiness(capabilities: DesktopCapability[]): {
  ready: number;
  attention: number;
  inactive: number;
} {
