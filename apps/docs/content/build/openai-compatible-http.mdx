...
The response lists `reference-react`. Requests may also use the accepted compatibility alias `reference-web`; both select the same Reference-agent implementation. Other configured profile IDs are rejected.
...
This is the simplest useful pattern for a browser app that already uses
OpenAI-style chat. The browser posts to your Next.js route. The route keeps
authentication and correlation on the server, then sends the request to its
configured Local Core bridge or remote runner service. The shared Execution
Protocol turns that request into a durable Kestrel run.
...
x-kestrel-model-id: reference-react
```
...
If your frontend needs continuity or debugging breadcrumbs, forward these
response headers from the configured Kestrel execution target:
