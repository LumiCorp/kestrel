...
summary: Copy the request bodies, response headers, cancellation behavior, and webhook mapping for a Kestrel-backed Next.js app.
section: build
...
<GuideNote step="Route reference">
Copy the exact request and response details for your Kestrel routes.
</GuideNote>
...
mapPayload(payload) {
  return {
    sessionId: payload.sessionId ?? "kestrel-session-001",
...
- [ ] route file imports the shared `kestrelAgent`
