...
Choose the Kestrel One source path when you need to inspect, extend, or run the application in a development environment. The source is versioned at `0.8.0`; Lumi hosting is a separate invitation-only access path.

[Get Kestrel One source](/kestrel-one/source-and-hosting)

## Use the terminal

Install `@kestrel-agents/kestrel@0.8.2` globally with npm on a release-verified platform, open the intended project directory, and start the TUI.
...
- The npm CLI/TUI reports `0.8.2`, becomes ready, and finishes one bounded project task.
