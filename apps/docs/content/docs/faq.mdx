...
Use the versions in the [compatibility matrix](/reference/compatibility). Exact first-party dependency pins must match the owning package manifest because Runtime, Protocol, SDK, Memory, adapters, and clients exchange exact fields, events, terminal states, approvals, and recovery decisions. The current public package line is 0.8.5, but independently released products such as Desktop can carry a different version. Kestrel does not promise 0.7/0.8 shims.
