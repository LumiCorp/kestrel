...
Every surface has a product version. A hosted production claim also needs the real Vercel deployment and migration result and, for each manually changed runtime target, its operator-selected image tag and provider result. One target's result does not prove another target changed.
