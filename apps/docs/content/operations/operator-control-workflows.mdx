...
Before acting, record the explicit Chat, Plan, or Build interaction mode, the current cursor, terminal/waiting state, and any exact pending-request or recovery-option identifier.
...
const inbox = await client.getOperatorInbox({ sessionId: "kestrel-session-001" }, context);
...
- inspect the authoritative Mission Control Project
- execute a versioned Mission Control action
- answer the exact pending request or recovery option

This gives browser users the same run controls as the CLI alongside run details and artifacts.

For project work, attempts, evidence, review, and acceptance, continue to [Mission Control review and state](/operate/review-and-state).
...
5. verify the resulting event, cursor, and terminal or waiting state belong to the same work
