...
The npm installation must report `0.8.7`. Resolve provider, Local Core, or workspace readiness failures before starting real work.
...
The Runtime/CLI [`v0.8.5` GitHub release](https://github.com/LumiCorp/kestrel/releases/tag/v0.8.5) provides the most recent standalone archive, `kestrel-cli-0.8.5-darwin-arm64.tar.gz`, and its checksum. It reports version `0.8.5`; use npm for the current Runtime/CLI 0.8.7. Verify the checksum, unpack it into an owned location, and ensure it is earlier on `PATH` than an older installation.
