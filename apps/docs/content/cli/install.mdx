...
The version must report `0.8.0`. Resolve provider, Local Core, or workspace readiness failures before starting real work.
...
The unified GitHub release also provides `kestrel-cli-0.8.0-darwin-arm64.tar.gz` and its checksum. Verify the checksum, unpack it into an owned location, and ensure it is earlier on `PATH` than an older installation.
