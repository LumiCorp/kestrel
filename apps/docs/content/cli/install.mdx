...
The npm installation must report `0.8.5`. Resolve provider, Local Core, or workspace readiness failures before starting real work.
...
The Runtime/CLI [`v0.8.5` GitHub release](https://github.com/LumiCorp/kestrel/releases/tag/v0.8.5) also provides `kestrel-cli-0.8.5-darwin-arm64.tar.gz` and its checksum. Verify the checksum, unpack it into an owned location, and ensure it is earlier on `PATH` than an older installation.
