...
```bash
pnpm add @kestrel-agents/next@0.8.4 @kestrel-agents/sdk@0.8.4
```
...
Use compatibility mode to preserve an existing client. Move to route helpers when the application needs typed sessions, waiting/recovery, terminal results, Memory, and operator behavior.
...
Move deeper into the SDK and native protocol when the app needs operator inbox/thread/control, Mission Control, exact recovery/approval decisions, or filtered durable subscriptions. That is the line between "proxying Kestrel" and "building directly on Kestrel."

Waiting and cancellation remain terminal/runtime states rather than assistant messages. Route helpers propagate request cancellation and return the validated terminal result; retain its session/run correlation when persisting UI state.
