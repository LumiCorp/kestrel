...
```bash
pnpm add @kestrel-agents/conversation@0.8.5
```
...
After ownership is known, the host message array is the durable transcript
preference. The projector preserves that order unless the turn input or a linked
interaction proves that another message must appear first. Roles and UUIDs do
