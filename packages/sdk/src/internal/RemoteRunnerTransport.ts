        ...
        if (event !== undefined) {
          ...
          if (acceptedOperatorControl && event.type === "operator.controlled") {
            ...
            if (acceptedOperatorIdentity === undefined) {
              ...
              this.emitEvent(makeSyntheticRunnerError(commandId, {
                code: "RUNNER_PROTOCOL_ERROR",
                message: "Remote runner emitted operator control with mismatched request, session, thread, run, or mode-resolution identity.",
          ...
          if (
            ...
            this.emitEvent(makeSyntheticRunnerError(commandId, {
              code: "RUNNER_PROTOCOL_ERROR",
              message: "Remote runner emitted a resumed run event with mismatched session, thread, or run identity.",
              details: { status: response.status, eventType: event.type },
            }));
            return false;
          }
