...
assertNonEmptyString(manifest.license, "packages/observability/package.json must declare license.");
assert.ok(
  typeof manifest.repository === "object" && manifest.repository !== null,
  "packages/observability/package.json must declare repository metadata.",
);
assertNonEmptyString(manifest.homepage, "packages/observability/package.json must declare homepage.");
assert.ok(
  typeof manifest.bugs === "object" && manifest.bugs !== null,
  "packages/observability/package.json must declare bugs metadata.",
);

const packDir = mkdtempSync(path.join(os.tmpdir(), "kestrel-observability-pack-"));
const extractDir = mkdtempSync(path.join(os.tmpdir(), "kestrel-observability-extract-"));
const fixtureDir = mkdtempSync(path.join(os.tmpdir(), "kestrel-observability-fixture-"));
const storeDir = mkdtempSync(path.join(os.tmpdir(), "kestrel-observability-store-"));
...
try {
  ...
  const tarballName = readdirSync(packDir).find((entry) => entry.startsWith("kestrel-agents-observability-") && entry.endsWith(".tgz"));
  assert.ok(tarballName, "pnpm pack did not produce an observability tarball.");
  const sdkTarballName = readdirSync(packDir).find((entry) => entry.startsWith("kestrel-agents-sdk-") && entry.endsWith(".tgz"));
  assert.ok(sdkTarballName, "pnpm pack did not produce an SDK tarball for observability dependency checks.");
  const protocolTarballName = readdirSync(packDir).find((entry) => entry.startsWith("kestrel-agents-protocol-") && entry.endsWith(".tgz"));
  assert.ok(protocolTarballName, "pnpm pack did not produce a protocol tarball for observability dependency checks.");
  ...
  assert.ok(tarEntries.includes("package/README.md"), "packed observability tarball is missing README.md.");
  assert.ok(tarEntries.includes("package/LICENSE"), "packed observability tarball is missing LICENSE.");
  assert.ok(tarEntries.includes("package/dist/index.js"), "packed observability tarball is missing dist/index.js.");
  assert.ok(tarEntries.includes("package/dist/otel.js"), "packed observability tarball is missing dist/otel.js.");
  assert.ok(tarEntries.includes("package/dist/otel.d.ts"), "packed observability tarball is missing dist/otel.d.ts.");
  ...
  writeFileSync(path.join(fixtureDir, "package.json"), JSON.stringify({
    name: "kestrel-observability-release-check",
    private: true,
    type: "module",
