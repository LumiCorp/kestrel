...
function readOptionalApprovalPack(
...
): "dev" | "isolated_code" | "ci_bot" | "hosted_workspace" | "production" | undefined {
  if (value === undefined) {
    return;
  }
  if (
  ...
  throw new Error(
    `Unsupported approval pack '${value}'. Expected dev|isolated_code|ci_bot|hosted_workspace|production.`,
  );
}
