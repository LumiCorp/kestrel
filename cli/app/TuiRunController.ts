...
import {
  ...
  type ResolvedTuiQueuedEvidence,
...
export class TuiRunController {
  ...
  private async startCapturedTurn(
    ...
    const reservedRunId = resumeRequestId === undefined && queueSubmission
      ? `runtime:${submissionMessageId}`
      : undefined;
    if (
      input.resumeBlockedRun !== true
    ...
    try {
      ...
      if (response.type === "conversation.message.routed") {
        ...
        let currentQueueGraph = normalizeTuiQueueGraph(currentSession);
        if (
          pendingQueueSubmission !== undefined
          && routedRunId !== undefined
          && routedRunId !== pendingQueueSubmission.runId
        ) {
          currentQueueGraph = replaceTuiQueueRunIdentity(
            currentQueueGraph,
            pendingQueueSubmission,
            routedRunId,
          );
          pendingQueueSubmission = findPendingQueueSubmission(
      ...
      const retainedRunSource = terminalRunId === undefined
        ? undefined
      ...
      const exactPendingSubmission = pendingSession.pendingRunMessageId === submissionMessageId
        ...
        && (
          ...
          || (
            pendingSession.pendingRunId === undefined
            && (
              pendingSession.acceptedRunId === undefined
              || retainedRunSource === submissionMessageId
            )
          )
        );
      const exactQueuedEvidence = queueSubmission
        && terminalRunId !== undefined
        && terminalOutput !== undefined
        ? resolveExactTuiQueuedEvidence(pendingSession, {
            runId: terminalRunId,
            threadId,
            messageId: submissionMessageId,
          })
        : undefined;
      const exactResultlessQueuedRejection = queueSubmission
        && response.type === "run.failed"
        && terminalOutput === undefined
        && terminalRunId !== undefined
        ? pendingSession.pendingQueueSubmissions?.find((submission) =>
            submission.runId === terminalRunId
            && submission.messageId === submissionMessageId
      ...
      if (
        ...
        || (
          ...
          && exactQueuedEvidence === undefined
          && exactAlreadyAcceptedSubmission === false
          && exactResultlessQueuedRejection === undefined
        )
      ...
      const responseCanInstallCurrentLifecycle = exactQueuedEvidence === undefined
        || queuedEvidenceCanReplaceAcceptedRun(normalizedPendingSession, exactQueuedEvidence);
      const settlePendingQueueGraph = (
        status?: "COMPLETED" | "FAILED" | undefined,
      ) => {
        if (exactQueuedEvidence === undefined) {
          return exactResultlessQueuedRejection === undefined
            ? pendingQueueGraph
            : {
                ...pendingQueueGraph,
                pendingQueueSubmissions: removePendingQueueSubmission(
                  pendingQueueGraph.pendingQueueSubmissions,
                  exactResultlessQueuedRejection,
                ),
              };
        }
        let acceptedGraph = responseCanInstallCurrentLifecycle
          ? advanceTuiQueueAuthority(pendingQueueGraph, exactQueuedEvidence)
          : {
              ...pendingQueueGraph,
              pendingQueueSubmissions: removePendingQueueSubmission(
                pendingQueueGraph.pendingQueueSubmissions,
        ...
        if (status !== undefined) {
          acceptedGraph = {
            ...
            queuedRunReservations: removeQueuedRunReservation(
              ...
              toQueuedRunReservation(exactQueuedEvidence),
            ),
          };
        }
        ...
        await setResponseSessionState({
          ...
          ...(responseCanInstallCurrentLifecycle
            ? {
                ...
                acceptedRunMessageId: submissionMessageId,
                acceptedRunThreadId: threadId,
                acceptedRunPredecessorId: exactQueuedEvidence === undefined
                  ? undefined
                  : durableAcceptedQueuePredecessorId(exactQueuedEvidence),
              }
            : {}),
      ...
      if (queueSubmission) {
        ...
        let recoverySession = readSubmittingSession();
        let recoveredQueueEvidence = queueSubmission && recoverySession !== undefined
          ? resolveExactTuiQueuedEvidence(recoverySession, {
              runId: reservedRunId!,
              threadId,
              messageId: submissionMessageId,
            })
        ...
        if (
          ...
          && recoveredRouteRunId !== recoveredQueueEvidence.runId
        ) {
          const rebound = replaceTuiQueueRunIdentity(
            normalizeTuiQueueGraph(recoverySession),
            recoveredQueueEvidence,
            recoveredRouteRunId,
          );
          await setResponseSessionState({
            pendingQueueSubmissions: rebound.pendingQueueSubmissions,
            queuedRunReservations: rebound.queuedRunReservations,
            terminalQueuedRuns: rebound.terminalQueuedRuns,
            updatedAt: new Date().toISOString(),
          });
          recoverySession = readSubmittingSession();
          recoveredQueueEvidence = recoverySession === undefined
            ? undefined
            : resolveExactTuiQueuedEvidence(recoverySession, {
                runId: recoveredRouteRunId,
                threadId,
                messageId: submissionMessageId,
              });
        }
        const exactRecoveredRoute = recoveredRoute?.disposition === "queued"
          ? recoveredRouteRunId === undefined || recoveredQueueEvidence !== undefined
          : recoveredRouteRunId !== undefined
        ...
        if (
          ...
          const queueEvidenceBeforeRecovery = sessionBeforeRecovery === undefined
            ? undefined
            : resolveExactTuiQueuedEvidence(sessionBeforeRecovery, {
                runId: recoveredRouteRunId ?? reservedRunId!,
                threadId,
                messageId: submissionMessageId,
              });
          ...
          if (recoveredRoute.disposition === "queued") {
            ...
            if (pendingQueueSubmission === undefined) return false;
            await setResponseSessionState({
              started: true,
            ...
            const recoveredLifecycleCommitted = await setResponseSessionState({
              ...
              acceptedRunPredecessorId: recoveredLifecycleConflicts
                ...
                : durableAcceptedQueuePredecessorId(queueEvidenceBeforeRecovery!),
              ...
              terminalQueuedRuns: terminalStatus === undefined
                ...
                : appendTerminalQueuedRun(currentSession?.terminalQueuedRuns, {
                    ...toQueuedRunReservation(queueEvidenceBeforeRecovery!),
                    status: terminalStatus,
                  }),
      ...
      if (
        ...
        const recoveredRoute = recoveredRoutes.length === 1 ? recoveredRoutes[0] : undefined;
        const finalRecoverySession = readSubmittingSession();
        const finalRecoveredQueueEvidence = queueSubmission && finalRecoverySession !== undefined
          ? resolveExactTuiQueuedEvidence(finalRecoverySession, {
              runId: reservedRunId!,
              threadId,
              messageId: submissionMessageId,
            })
        ...
        const exactAcceptedRun = [...(
          ...
          sourceMessageId === submissionMessageId
          && (readSubmittingSession()?.pendingRunId === undefined
            || readSubmittingSession()?.pendingRunId === runId
            || readSubmittingSession()?.pendingRunId?.startsWith("tui-foreground:") === true)
        );
        const recoveredRouteRunId = recoveredRoute?.runId;
        const recoveredRunStatus = recoveredRouteRunId === undefined || recoveredView === undefined
          ? undefined
          : exactRunStatusFromView(recoveredView, recoveredRouteRunId);
        const exactRecoveredRoute = recoveredRouteRunId !== undefined
          && recoveredRunStatus !== undefined
          && (
        ...
        if (
          ...
          await setResponseSessionState({
            ...
            acceptedRunMessageId: submissionMessageId,
            acceptedRunThreadId: threadId,
            acceptedRunPredecessorId: finalRecoveredQueueEvidence === undefined
              ? undefined
              : durableAcceptedQueuePredecessorId(finalRecoveredQueueEvidence),
            updatedAt: new Date().toISOString(),
          });
          if (submittingSessionIsActive()) this.context.uiStore.patch({
            running: authoritativeRunActive,
    ...
    if (
      ...
      const exactQueuedForegroundStart = session?.delegation === undefined
        ...
        && (
          ...
          || findQueuedRunReservationByMessage(
            session?.queuedRunReservations,
            event.threadId,
            event.payload.sourceMessageId,
          ) !== undefined
          || findPendingQueueSubmissionByMessage(
      ...
      const queuedBackgroundEvidence = event.threadId !== undefined
        && event.payload.sourceMessageId !== undefined
        && session !== undefined
        ? resolveExactTuiQueuedEvidence(session, {
          runId: event.runId,
          threadId: event.threadId,
          messageId: event.payload.sourceMessageId,
        })
        : undefined;
      const exactQueuedBackgroundStart = queuedBackgroundEvidence !== undefined
        && queuedBackgroundEvidence.source !== "tombstone";
      const exactAcceptedBackgroundStart = event.threadId !== undefined
        && event.payload.sourceMessageId !== undefined
        && session?.acceptedRunId === event.runId
        && session.acceptedRunMessageId === event.payload.sourceMessageId
        && session.acceptedRunThreadId === event.threadId;
      ...
      if (
        ...
        void this.runQueueJournalTransaction(eventSessionId, async () => {
          ...
          const pendingQueueMessage = findPendingQueueSubmissionByMessage(
            current?.pendingQueueSubmissions,
            eventThreadId,
            sourceMessageId,
          );
          const queuedMessage = findQueuedRunReservationByMessage(
            current?.queuedRunReservations,
            eventThreadId,
            sourceMessageId,
          );
          const localQueueEvidence = pendingQueueMessage ?? queuedMessage;
          if (
            current !== undefined
            && localQueueEvidence !== undefined
            && localQueueEvidence.runId !== eventRunId
          ) {
            const rebound = replaceTuiQueueRunIdentity(
              normalizeTuiQueueGraph(current),
              localQueueEvidence,
              eventRunId,
            );
            await this.context.setSessionState(eventSessionId, {
              pendingQueueSubmissions: rebound.pendingQueueSubmissions,
              queuedRunReservations: rebound.queuedRunReservations,
              terminalQueuedRuns: rebound.terminalQueuedRuns,
              updatedAt: new Date().toISOString(),
      ...
      void this.runQueueJournalTransaction(output.sessionId, async () => {
        ...
        if (queuedTerminalEvidence && event.threadId !== undefined) {
          const synchronizedQueuedTerminal = await this.context.syncForegroundQueuedTerminal({
            ...
            threadId: event.threadId,
            runId: output.runId,
            messageId: queuedTerminalEvidence.messageId,
  ...
  private terminalEventOwnsCurrentLifecycle(
    ...
    const evidence = resolveExactTuiQueuedEvidence(session, { runId, threadId });
  ...
  private foregroundQueuedTerminalEvidence(
    ...
    return resolveExactTuiQueuedEvidence(session, { runId, threadId });
  }
...
function findPendingQueueSubmissionByMessage(
  submissions: TuiSessionMeta["pendingQueueSubmissions"],
  threadId: string,
  messageId: string,
): NonNullable<TuiSessionMeta["pendingQueueSubmissions"]>[number] | undefined {
...
function findQueuedRunReservationByMessage(
  reservations: TuiSessionMeta["queuedRunReservations"],
  threadId: string,
  messageId: string,
): NonNullable<TuiSessionMeta["queuedRunReservations"]>[number] | undefined {
