...
} from "../../src/terminal/UserTerminalService.js";
...
import {
  ...
  type ProposedWorkspaceReviewFinding,
} from "../../src/review/WorkspaceReviewService.js";
import type {
...
interface RuntimeBootstrap {
  ...
  prepareHostedMcpRuntime?:
    | ((
        input: Pick<
          RunTurnInput,
          "runId" | "sessionId" | "mcpContext" | "mcpAuthorization"
        >,
      ) => Promise<unknown>)
    | undefined;
...
export interface RuntimeFactoryWithStoreOptions {
  resolveEnvironment?:
    | ((profile: TuiProfile) => KestrelRuntimeEnvironment)
    | undefined;
...
export class KestrelChatRuntime {
  ...
  private readonly projectRuntimeService:
    | ProductProjectRuntimeService
    | undefined;
  private readonly workspaceCheckpointService:
    | WorkspaceCheckpointService
    | undefined;
  private readonly runtimeWorkspaceCheckpointService:
    | RuntimeWorkspaceCheckpointService
    | undefined;
  ...
  private readonly workspaceFeedbackService:
  ...
  private readonly workspaceValidationService:
  ...
  constructor(
    ...
    this.projectRuntimeService =
      bootstrap.taskGraphStore !== undefined &&
      bootstrap.projectStore !== undefined
    ...
    this.runtimeWorkspaceCheckpointService =
      bootstrap.workspaceCheckpointService !== undefined &&
      bootstrap.projectStore !== undefined
        ? new RuntimeWorkspaceCheckpointService({
            ...
            resolver: new WorkspaceContextResolver({
              ...
              updateManagedWorktreeBinding: async ({ sessionId, binding }) => {
                await this.kestrel.updateManagedWorktreeBinding(
                  sessionId,
                  binding,
                );
              },
    ...
    this.workspaceFeedbackReady =
      bootstrap.workspaceFeedbackReady ?? Promise.resolve();
    this.workspaceReviewService = bootstrap.workspaceReviewService;
    this.workspaceReviewReady =
      bootstrap.workspaceReviewReady ?? Promise.resolve();
    this.workspaceValidationService = bootstrap.workspaceValidationService;
    this.workspaceValidationReady =
      bootstrap.workspaceValidationReady ?? Promise.resolve();
    ...
    this.turnCoordinator = new RuntimeTurnCoordinatorService({
      ...
      ...(this.threadRuntime !== undefined
        ...
        : {}),
      directRun: async (event, runOptions) =>
        this.kestrel.run(event, runOptions),
      getSession: async (sessionId) =>
        (await this.kestrel.getSession(sessionId)) ?? undefined,
  ...
  async runTurn(
    ...
    const effectiveInput = await applyActiveTaskRuntimeMetadata(
      normalizedInput,
      this.taskGraphStore,
    );
    const policyBoundWorkspace = applyRequiredManagedWorkspacePolicy(
      effectiveInput.workspace,
    );
    const authorizedInput: RunTurnInput = {
      ...effectiveInput,
      ...(effectiveInput.mcpAuthorization !== undefined &&
      effectiveInput.runId === undefined
        ? { runId: randomUUID() }
        : {}),
    ...
    try {
      ...
      const result = await this.turnCoordinator.runTurn(
        persistableInput,
        options,
      );
  ...
  async describeSession(sessionId: string): Promise<
    | {
        ...
        latestCheckpointDisposition?:
          | OperatorCheckpointSummary["status"]
          | undefined;
        ...
        visibleTodos?:
          | import("../../src/runtime/visibleTodos.js").VisibleTodoState
          | undefined;
        contextPosture?: string | undefined;
        operatorPhase?:
          | import("../../src/orchestration/index.js").OperatorThreadView["operatorPhase"]
          | undefined;
        modelProvenance?:
          | import("../../src/replay/RunReplayService.js").ReplayModelProvenanceSummary
          | undefined;
  ...
  async getSessionState(
    sessionId: string,
  ): Promise<RuntimeSessionStateProjection | undefined> {
    const session = await this.kestrel.getSession(sessionId);
    if (session === null) {
      return;
    }
  ...
  async listOperatorRuns(
    input: {
      sessionId?: string | undefined;
      status?:
        | import("../../src/orchestration/contracts.js").OperatorRunStatus
        | undefined;
  ...
  async performProjectAction(input: ProductProjectAction) {
    return requireProductProjectRuntimeService(
      this.projectRuntimeService,
    ).performProjectAction(input);
  }
  ...
  async applyWorkspacePromotion(input: {
    ...
    if (this.workspaceValidationService !== undefined)
      await this.assertWorkspaceDeliveryReady(
        { sessionId: input.sessionId, threadId: input.sessionId },
        "promotion",
      );
    return requireRuntimeWorkspaceCheckpointService(
      this.runtimeWorkspaceCheckpointService,
  ...
  async readUserTerminal(input: {
    sessionId: string;
    terminalId: string;
    cursor?: number | undefined;
  }): Promise<UserTerminalReadResult> {
    await this.userTerminalReady;
    return requireUserTerminalService(this.userTerminalService).read(input);
  }

  async writeUserTerminal(input: {
    sessionId: string;
    terminalId: string;
    data: string;
  }): Promise<UserTerminalRecord> {
  ...
  async resizeUserTerminal(input: {
    sessionId: string;
    terminalId: string;
    cols: number;
    rows: number;
  ...
  async inspectWorkspaceChanges(input: {
    ...
    if (input.scope.kind === "latest_turn") {
      ...
      const service = requireWorkspaceChangeService(
        this.workspaceChangeService,
      );
      ...
      if (!(checkpointService && threadRuntime))
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_TURN_SCOPE_UNAVAILABLE",
          "Conversation-turn checkpoints are unavailable.",
          {
            subsystem: "workspace",
      ...
      const turn = requestedTurnId
        ? turns.find((candidate) => candidate.turnId === requestedTurnId)
        : turns[0];
      if (!turn)
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_TURN_NOT_FOUND",
          "No conversation turn is available for this coding thread.",
          {
            subsystem: "workspace",
      ...
      const segments = await threadRuntime.listConversationTurnSegments(
        turn.turnId,
      );
      const runIds = new Set(
        [
          ...
          turn.activeRunId,
          turn.terminalRunId,
          ...segments.map((segment) => segment.runId),
        ].filter((value): value is string => typeof value === "string"),
      );
      const checkpoints = (
        await checkpointService.list({ sessionId: input.sessionId })
      )
        .filter(
          (checkpoint) =>
            checkpoint.runId !== undefined && runIds.has(checkpoint.runId),
        )
        .sort((a, b) => a.createdAt.localeCompare(b.createdAt));
      if (checkpoints.length === 0)
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_TURN_CHECKPOINTS_NOT_FOUND",
          "The selected turn has no workspace mutation checkpoints.",
          {
            subsystem: "workspace",
            classification: "state",
            recoverable: true,
      ...
      const workspaceRoot = await this.resolveAuthoritativeWorkspace(input);
      if (
        path.resolve(source.workspaceRoot) !== path.resolve(workspaceRoot) ||
        path.resolve(target.workspaceRoot) !== path.resolve(workspaceRoot)
      )
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_TURN_AUTHORITY_MISMATCH",
          "Turn checkpoints do not belong to the authoritative thread workspace.",
          {
            subsystem: "workspace",
    ...
    if (input.scope.kind === "latest_run") {
      ...
      if (!checkpointService)
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_RUN_SCOPE_UNAVAILABLE",
          "Run checkpoints are unavailable.",
          {
            subsystem: "workspace",
            classification: "configuration",
            recoverable: true,
      ...
      if (!(runId && permittedRunIds.includes(runId)))
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_RUN_NOT_FOUND",
          "No checkpointed run is available for this coding thread.",
          {
            subsystem: "workspace",
            classification: "state",
            recoverable: true,
      ...
      if (checkpoints.length === 0)
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_RUN_CHECKPOINTS_NOT_FOUND",
          "The selected run has no workspace mutation checkpoints.",
          {
            subsystem: "workspace",
            classification: "state",
            recoverable: true,
      ...
      if (
        ...
        throw createRuntimeFailure(
          "WORKSPACE_CHANGE_RUN_AUTHORITY_MISMATCH",
          "Run checkpoints do not belong to the authoritative thread workspace.",
          {
            subsystem: "workspace",
            classification: "authorization",
            recoverable: false,
    ...
    if (input.scope.kind === "promotion") {
      ...
      if (
        path.resolve(promotion.managedWorktreeRoot) !==
        path.resolve(authoritative)
      )
        throw createRuntimeFailure(
      ...
      return requireWorkspaceChangeService(
        this.workspaceChangeService,
      ).inspectGitRange({
        ...input,
        workspaceRoot: promotion.managedWorktreeRoot,
        baseRef: promotion.baseHead,
        ...(preview.preview.candidateFingerprint
          ? { candidateFingerprint: preview.preview.candidateFingerprint }
          : {}),
      });
    }
    const workspaceRoot = await this.resolveAuthoritativeWorkspace(input);
  ...
  async addWorkspaceFeedback(input: {
    ...
    assertCandidateFingerprint(
      input.candidateFingerprint,
      actual.candidateFingerprint,
    );
    await this.workspaceFeedbackReady;
    return requireWorkspaceFeedbackService(this.workspaceFeedbackService).add(
      input,
    );
  }
  ...
  async removeWorkspaceFeedback(input: {
    ...
    await this.workspaceFeedbackReady;
    return requireWorkspaceFeedbackService(
      this.workspaceFeedbackService,
    ).remove(input);
  }
  ...
  async submitWorkspaceFeedback(input: {
    ...
    await this.workspaceFeedbackReady;
    const service = requireWorkspaceFeedbackService(
      this.workspaceFeedbackService,
    );
  ...
  async runWorkspaceReview(input: {
    ...
    if (
      mode === "current_thread" &&
      (input.reviewerProfileId || input.reviewerModel)
    )
      throw createRuntimeFailure(
        "WORKSPACE_REVIEW_PROFILE_UNAVAILABLE",
        "Dedicated reviewer profile selection is available through detached review threads.",
        {
          subsystem: "review",
          classification: "configuration",
          recoverable: true,
    ...
    try {
      const repositoryInstructions = await readRepositoryReviewInstructions(
        candidate.repoRoot,
      );
      if (mode === "detached_thread") {
        ...
        if (!threadRuntime)
          throw createRuntimeFailure(
            "WORKSPACE_REVIEW_DETACHED_UNAVAILABLE",
            "Detached review threads are unavailable.",
            {
              subsystem: "review",
              classification: "configuration",
              recoverable: true,
        ...
        const handle = await threadRuntime.spawnChildThread({
          ...
          prompt: workspaceReviewPrompt(
            candidate.diff,
            candidate.candidateFingerprint,
          ),
          rolePrompt: [
            "You are a bounded, strictly read-only code reviewer.",
      ...
      return await service.complete({
        ...
        candidateFingerprint: candidate.candidateFingerprint,
        runId: result.output.runId,
        findings: parseWorkspaceReviewFindings(
          result.finalizedPayload ?? result.assistantText,
        ),
      });
    } catch (cause) {
      await service.fail({
  ...
  async listWorkspaceReviews(input: {
    sessionId: string;
    threadId: string;
  }): Promise<WorkspaceReviewSnapshot> {
    const actual = await this.inspectWorkspaceChanges({
    ...
    for (const review of service.records(input)) {
      try {
        ...
        if (
          ...
          review.mode === "detached_thread" &&
          review.delegationId &&
          this.threadRuntime
        ) {
          const delegation = (
            await this.threadRuntime.listDelegations(input.threadId)
          ).find((candidate) => candidate.delegationId === review.delegationId);
          if (delegation?.status === "COMPLETED" && delegation.result?.result) {
            if (current.candidateFingerprint !== review.candidateFingerprint)
              continue;
            try {
            ...
            await service.fail({
              ...
              error:
                ...
                `Detached review ${delegation.status.toLowerCase()}.`,
            });
        }
      } catch {
  ...
  async submitWorkspaceReviewFindings(input: {
    ...
    const actual = await this.inspectWorkspaceChanges({
      ...
      threadId: input.threadId,
      scope:
        input.request === "verify" && review.status === "stale"
          ? { kind: "uncommitted" }
          : review.scope,
    });
    assertCandidateFingerprint(
  ...
  async inspectWorkspaceValidation(input: {
    ...
    return requireWorkspaceValidationService(
      this.workspaceValidationService,
    ).inspect({
      ...input,
  ...
  async runWorkspaceValidation(input: {
    ...
    assertCandidateFingerprint(
      input.candidateFingerprint,
      changes.candidateFingerprint,
    );
    const service = requireWorkspaceValidationService(
      this.workspaceValidationService,
    );
    const base = {
      sessionId: input.sessionId,
  ...
  async cancelWorkspaceValidation(input: {
    ...
    await this.workspaceValidationReady;
    await requireWorkspaceValidationService(
      this.workspaceValidationService,
    ).cancel(input);
  ...
  async inspectWorkspaceGit(input: {
    sessionId: string;
    threadId: string;
    ...
    return requireWorkspaceGitService(this.workspaceGitService).inspect({
      ...
      candidateFingerprint: changes.candidateFingerprint,
      validationReadiness:
        validation.candidateFingerprint === changes.candidateFingerprint
  ...
  async performWorkspaceGitAction(input: {
    ...
    if (
      input.action.kind === "commit" ||
      input.action.kind === "push" ||
      input.action.kind === "pr_ready" ||
      (input.action.kind === "pr_create" && !input.action.draft)
    )
      await this.assertWorkspaceDeliveryReady(
        input,
        input.action.kind === "pr_ready" || input.action.kind === "pr_create"
    ...
    switch (input.action.kind) {
      case "branch_create":
        await service.createBranch({
          ...base,
          branchName: input.action.branchName,
        });
        break;
      case "fetch":
        await service.fetch({ ...base, remote: input.action.remote });
        break;
      case "commit":
      ...
      case "push":
        await service.push({
          ...base,
          remote: input.action.remote,
          branch: input.action.branch,
          setUpstream: input.action.setUpstream,
        });
        break;
      case "pr_create":
        await service.createPullRequest({
          ...
          baseBranch: input.action.baseBranch,
          draft: input.action.draft,
        });
        break;
      case "pr_ready":
        await service.markPullRequestReady({
          ...base,
      ...
      case "pr_comment":
        await service.commentOnPullRequest({
          ...
          ...(input.action.line ? { line: input.action.line } : {}),
          ...(input.action.side ? { side: input.action.side } : {}),
        });
        break;
    }
  ...
  private async assertWorkspaceDeliveryReady(
    input: { sessionId: string; threadId: string },
    operation: string,
  ): Promise<void> {
    const validation = await this.inspectWorkspaceValidation(input);
    if (validation.readiness.state !== "ready")
      throw createRuntimeFailure(
        "WORKSPACE_DELIVERY_VALIDATION_REQUIRED",
        `Current candidate validation is ${validation.readiness.state.replace("_", " ")}; ${operation} requires fresh passing evidence.`,
        {
          subsystem: "validation",
          classification: "state",
          recoverable: true,
          candidateFingerprint: validation.candidateFingerprint,
          readiness: validation.readiness.state,
          operation,
        },
      );
  }
  ...
  async performProjectReviewAction(input: {
    ...
    return requireProductProjectRuntimeService(
      this.projectRuntimeService,
    ).performProjectReviewAction(input);
  }
  ...
  async performOperatorAction(input: {
    ...
    threadId: string;
    inbox?:
      | import("../../src/orchestration/index.js").OperatorInboxSnapshot
      | undefined;
    view?:
      | import("../../src/orchestration/index.js").OperatorThreadView
      | undefined;
    ...
    if (
      ...
      const requestId =
        input.requestId ??
        (await threadRuntime.getThreadStatus(input.threadId))?.openRequests[0]
          ?.requestId;
      ...
      if (
        ...
        (input.actionValue !== "accept" && input.actionValue !== "defer")
      ) {
        throw createRuntimeFailure(
      ...
      if (
        ...
        throw createRuntimeFailure(
          "OPERATOR_FOLLOW_UP_INPUT_INVALID",
          "Editing a follow-up requires followUpId and message.",
        );
      }
      await threadRuntime.editFollowUp({
        threadId: input.threadId,
        followUpId,
        message,
      });
      ...
      if (followUpId === undefined || followUpId.length === 0) {
        throw createRuntimeFailure(
          "OPERATOR_FOLLOW_UP_INPUT_INVALID",
          "Cancelling a follow-up requires followUpId.",
        );
      }
      await threadRuntime.cancelFollowUp({
        threadId: input.threadId,
        followUpId,
      });
  ...
  async cancelActiveRun(
    ...
    if (thread !== undefined && this.threadRuntime !== undefined) {
      await this.threadRuntime.pauseFollowUpQueue({
  ...
  async getRetainedProviderReasoning(input: {
    runId: string;
    sessionId: string;
    actorRole: string;
  ...
  async deleteRetainedProviderReasoning(input: {
    runId: string;
    sessionId: string;
    actorRole: string;
    actorId?: string | undefined;
  ...
  private async resolveAuthoritativeWorkspace(input: {
    sessionId: string;
    threadId: string;
  }): Promise<string> {
    ...
    if (
      view == null ||
      view.thread.sessionId !== input.sessionId ||
      view.workspace === undefined
    ) {
      throw createRuntimeFailure(
        "USER_TERMINAL_WORKSPACE_UNAVAILABLE",
        "Authoritative thread workspace is unavailable for this terminal.",
        {
          subsystem: "terminal",
          classification: "authorization",
          recoverable: true,
...
function createRuntimeWithStore(
  ...
  const userTerminalService = enableUserTerminals
    ...
    : undefined;
  const workspaceChangeService = enableWorkspaceChanges
    ? new WorkspaceChangeService()
    : undefined;
  const workspaceFeedbackService = enableWorkspaceChanges
    ? new WorkspaceFeedbackService(
        path.join(
          resolveKestrelCoreHome(runtimeEnv).homePath,
          "review",
          "feedback.json",
  ...
  const registration = registerAgent(kestrel, profile.agent, {
    ...(profile.modelProvider !== undefined
      ? { agentProvider: profile.modelProvider }
      : {}),
    ...
    ...(managedTaskWorktreeService !== undefined
      ? {
          managedWorktreeProposalProvider: (
            request: Parameters<ManagedTaskWorktreeService["prepare"]>[0],
  ...
  const threadedTurnExecutor = new RuntimeThreadedTurnExecutor({
    ...
    defaults: {
      ...
      defaultToolAllowlist: profile.toolAllowlist ?? [
        ...DEFAULT_BALANCED_TOOL_ALLOWLIST,
      ],
  ...
  return {
    ...
    ...(workspaceFeedbackService !== undefined
      ? { workspaceFeedbackService }
      : {}),
...
function requireUserTerminalService(
  service: UserTerminalService | undefined,
): UserTerminalService {
  if (service === undefined) {
    throw createRuntimeFailure(
      "USER_TERMINAL_UNAVAILABLE",
...
function requireWorkspaceChangeService(
  service: WorkspaceChangeService | undefined,
): WorkspaceChangeService {
  if (service === undefined) {
    throw createRuntimeFailure(
      "WORKSPACE_CHANGE_UNAVAILABLE",
...
function requireWorkspaceFeedbackService(
  service: WorkspaceFeedbackService | undefined,
): WorkspaceFeedbackService {
  if (service === undefined) {
    throw createRuntimeFailure(
      "WORKSPACE_FEEDBACK_UNAVAILABLE",
      "Workspace feedback is unavailable.",
      {
        subsystem: "review",
        classification: "configuration",
        recoverable: true,
...
function requireWorkspaceReviewService(
  service: WorkspaceReviewService | undefined,
): WorkspaceReviewService {
  if (service === undefined)
    throw createRuntimeFailure(
...
function requireWorkspaceGitService(
  service: WorkspaceGitService | undefined,
): WorkspaceGitService {
  if (service === undefined)
    throw createRuntimeFailure(
      "WORKSPACE_GIT_UNAVAILABLE",
      "Workspace Git delivery is unavailable.",
      {
        subsystem: "git",
        classification: "configuration",
        recoverable: true,
...
async function readRepositoryReviewInstructions(
  repoRoot: string,
): Promise<string[]> {
  const result: string[] = [];
  for (const name of [
    ...
    try {
      ...
      if (content.trim())
        result.push(
          `Repository review instructions from ${name}:\n${content.slice(0, 64 * 1024)}`,
        );
    } catch {
...
function parseWorkspaceReviewFindings(
  value: unknown,
): ProposedWorkspaceReviewFinding[] {
  let parsed = value;
  if (typeof value === "string") {
    ...
    try {
      ...
      throw createRuntimeFailure(
        "WORKSPACE_REVIEW_OUTPUT_INVALID",
  ...
  const candidate = Array.isArray(record?.findings)
    ? record
  ...
  if (!(candidate && Array.isArray(candidate.findings)))
    throw createRuntimeFailure(
      "WORKSPACE_REVIEW_OUTPUT_INVALID",
      "Reviewer output did not contain typed findings.",
      {
        subsystem: "review",
        classification: "contract",
        recoverable: true,
  ...
  return candidate.findings.map((entry) => {
    ...
    if (!finding)
      throw createRuntimeFailure(
        "WORKSPACE_REVIEW_OUTPUT_INVALID",
        "Reviewer returned a malformed finding.",
        {
          subsystem: "review",
          classification: "contract",
          recoverable: true,
...
function assertCandidateFingerprint(expected: string, actual: string): void {
  if (expected !== actual) {
    throw createRuntimeFailure(
      "WORKSPACE_FEEDBACK_STALE",
      "The workspace changed after this feedback was created. Refresh before continuing.",
      {
        subsystem: "review",
        classification: "state",
        recoverable: true,
